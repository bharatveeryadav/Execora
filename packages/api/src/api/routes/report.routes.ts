 ...
 *
 * ── Accounting reports ───────────────────────────────────────────────────────
 * GET  /api/v1/reports/aging             — Receivables aging (0-30/31-60/61-90/91+ days)
 * GET  /api/v1/reports/payment-velocity  — Avg days-to-pay per customer (?from=&to=)
 * GET  /api/v1/reports/day-book          — Daily journal (?date=YYYY-MM-DD)
 * GET  /api/v1/reports/outstanding       — All unpaid invoices (?customerId=)
 * GET  /api/v1/customers/:id/statement   — Customer account statement (?from=&to=)
 */
...
function resolveDateRange(
  ...
  if (q.fy) {
    ...
    if (isNaN(from.getTime()) || isNaN(to.getTime())) {
      reply
        .code(400)
        .send({ error: "Invalid date format — use YYYY-MM-DD or fy=2025-26" });
      return null;
    }
  } else {
...
export async function reportRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (
      ...
      const report = await gstr1Service.getGstr1Report(
        tenantId,
        range.from,
        range.to,
      );
  ...
  fastify.get(
    ...
    async (
      ...
      const report = await gstr1Service.getItemwisePnlReport(
        tenantId,
        range.from,
        range.to,
      );
  ...
  fastify.get(
    ...
    async (
      ...
      const report = await gstr1Service.getPnlReport(
        tenantId,
        range.from,
        range.to,
        compareFrom,
  ...
  fastify.get(
    ...
    async (
      ...
      return reply
        ...
        .header(
          "Content-Disposition",
          `attachment; filename="PnL_${fromLabel}_${toLabel}.pdf"`,
        )
        .send(pdf);
    },
  );
  ...
  fastify.get(
    ...
    async (
      ...
      return reply
        ...
        .header(
          "Content-Disposition",
          `attachment; filename="PnL_${fromLabel}_${toLabel}.csv"`,
        )
        .send("\uFEFF" + csv);
    },
  );
  ...
  fastify.post(
    ...
    async (request: FastifyRequest<{ Body: EmailBody }>, reply) => {
      const tenantId = request.user!.tenantId;
      ...
      if (!type || !from || !to || !email) {
        return reply
          .code(400)
          .send({ error: "Missing required fields: type, from, to, email" });
      }

      if (!emailService.isEnabled()) {
        return reply
          .code(503)
          .send({ error: "Email service not configured on this server" });
      }
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Querystring: { asOf?: string } }>) => {
      const tenantId = request.user!.tenantId;
      const asOf = request.query.asOf
        ? new Date(request.query.asOf)
        : undefined;
      return getAgingReport(tenantId, asOf);
    },
  );

  // ── GET /api/v1/reports/payment-velocity ──────────────────────────────────
  fastify.get(
    "/api/v1/reports/payment-velocity",
    async (
      request: FastifyRequest<{ Querystring: ReportQuerystring }>,
      reply,
    ) => {
      ...
      const range = resolveDateRange(request.query, reply);
      if (!range) return;
      return getPaymentVelocityReport(tenantId, range.from, range.to);
    },
  );
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Querystring: { date?: string } }>) => {
      ...
      const date = request.query.date
        ...
        : new Date();
      return getDayBook(tenantId, date);
    },
  );
  ...
  fastify.get(
    ...
    async (
      ...
      const tenantId = request.user!.tenantId;
      const { customerId, asOf, maxAgeDays } = request.query;
      return getOutstandingReceivables(tenantId, {
        customerId,
  ...
  fastify.get<{ Params: { id: string }; Querystring: ReportQuerystring }>(
    ...
    async (request, reply) => {
      ...
      const range = resolveDateRange(request.query, reply);
      if (!range) return;
      return getAccountStatement(tenantId, customerId, range.from, range.to);
    },
  );
}
