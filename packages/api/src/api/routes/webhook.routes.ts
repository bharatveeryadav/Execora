...
export async function webhookRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      const secret = ((tenant.settings as Record<string, unknown>) ?? {})[
        "razorpayWebhookSecret"
      ] as string | undefined;

      if (secret) {
        const sig = request.headers["x-razorpay-signature"] as
          | string
          | undefined;
      ...
      const pe =
        ((
          (body?.payload as Record<string, unknown>)?.payment as Record<
            string,
            unknown
          >
        )?.entity as Record<string, unknown>) ??
        ((
          (body?.payload as Record<string, unknown>)?.payment_link as Record<
            string,
            unknown
          >
        )?.entity as Record<string, unknown>) ??
        ...
        return reply.send(
          await processUpiPayment(
            tenantId,
            {
              ...
              method: (["upi", "card", "cash"].includes(methodRaw)
                ? methodRaw
                : "upi") as "upi" | "card" | "cash",
              source: "razorpay",
            },
            recId,
          ),
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      const saltKey = ((tenant.settings as Record<string, unknown>) ?? {})[
      ...
      if (typeof body?.response === "string") {
        try {
          data = JSON.parse(Buffer.from(body.response, "base64").toString());
        } catch {
          /* use raw */
        }
      }
      ...
      const code =
        (data?.code as string) ??
        ((data?.data as Record<string, unknown>)?.responseCode as string) ??
        "";
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      if (secret && sig && timestamp) {
        const expected = hmac256b64(secret, timestamp + JSON.stringify(body));
        if (sig !== expected) {
      ...
      const payment =
        ((body?.data as Record<string, unknown>)?.payment as Record<
          string,
          unknown
        >) ?? body;
      const customer =
        ((body?.data as Record<string, unknown>)?.customer as Record<
          string,
          unknown
        >) ?? {};
      const amtRs =
        typeof payment?.payment_amount === "number"
          ...
          : parseFloat(String(payment?.payment_amount ?? payment?.amount ?? 0));
      if (!amtRs || amtRs <= 0)
        return reply.code(400).send({ error: "Zero amount" });
      ...
      try {
        ...
        return reply.send(
          await processUpiPayment(
            ...
            {
              amount: amtRs,
              phone: ((customer?.customer_phone as string) ?? "").replace(
                /\D/g,
                "",
              ),
          ...
          await processUpiPayment(
            tenantId,
            {
              ...
              ref:
                ...
                `payu_${Date.now()}`,
              method:
                String(body?.mode ?? "UPI").toUpperCase() === "UPI"
                  ? "upi"
                  : "other",
              source: "payu",
            },
      ...
      try {
        ...
        return reply.send(
          await processUpiPayment(
            ...
            {
              amount: amtRs,
              phone: String(body?.MOBILE_NO ?? body?.CUST_ID ?? "").replace(
                /\D/g,
                "",
              ),
              name: "",
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      const salt = ((tenant.settings as Record<string, unknown>) ?? {})[
        "instamojoSalt"
      ] as string | undefined;
      const sig = request.headers["x-instamojo-signature"] as
        | string
        | undefined;
      ...
      if (salt && sig) {
        ...
        const mac = macFields.map((k) => String(body?.[k] ?? "")).join("|");
        const expected = crypto
      ...
      if (body?.status !== "Credit") return reply.send({ status: "ignored" });
        ...
        return reply.send(
          await processUpiPayment(
            tenantId,
            {
              ...
              ref: (body?.payment_id as string) ?? `imo_${Date.now()}`,
              method: "upi",
              source: "instamojo",
            },
            recId,
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      const sigHeader = request.headers["stripe-signature"] as
        | string
        | undefined;
      ...
      const pi = (body?.data as Record<string, unknown>)?.object as Record<
        string,
        unknown
      >;
        ...
        return reply.send(
          await processUpiPayment(
            tenantId,
            {
              ...
              ref: (pi?.id as string) ?? `str_${Date.now()}`,
              method: methodTypes.includes("upi")
                ? "upi"
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      if (!status.includes("SUCCESS")) return reply.send({ status: "ignored" });
      ...
      try {
        return reply.send(
          await processUpiPayment(
            ...
            {
              amount: amtRs,
              phone: String(data?.mobile ?? data?.mobileNumber ?? "").replace(
                /\D/g,
                "",
              ),
