...
import { inventoryMovementsRoutes } from "./routes/inventory-movements.routes";
...
export async function registerRoutes(fastify: FastifyInstance) {
  ...
  await fastify.register(demoRoutes); // GET /api/v1/demo-invoices|purchases|quotations
  ...
  await fastify.register(async (scope: FastifyInstance) => {
    ...
    await scope.register(complianceRoutes);
    await scope.register(inventoryMovementsRoutes);
    await scope.register(auditRoutes);
    await scope.register(subscriptionRoutes);
    await scope.register(posRoutes);
  });
