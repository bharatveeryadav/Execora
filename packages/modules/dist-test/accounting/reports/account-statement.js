 ...
 */
async function getAccountStatement(tenantId, customerId, from, to) {
    ...
    const [invBefore, payBefore, cnBefore] = await Promise.all([
        ...
        core_1.prisma.creditNote.aggregate({
            where: {
                tenantId,
                customerId,
                status: "issued",
    ...
    const openingBalance = round2(toN(invBefore._sum.total) -
    ...
    const entries = raw.map((ev) => {
        ...
        if (ev.type === "invoice")
            totalInvoiced = round2(totalInvoiced + ev.debit);
        if (ev.type === "payment")
            totalPaid = round2(totalPaid + ev.credit);
