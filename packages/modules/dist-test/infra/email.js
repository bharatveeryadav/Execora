...
class EmailService {
    ...
    async initialize() {
        try {
            ...
            if (!emailConfig) {
                core_1.logger.warn('Email service disabled - missing configuration');
                this.enabled = false;
                return;
            }
            ...
            this.enabled = true;
            core_1.logger.info({ emailFrom: emailConfig.from }, '✉️ Email service initialized successfully');
        }
        catch (error) {
            core_1.logger.error({ error: error.message }, '❌ Failed to initialize email service');
            this.enabled = false;
        }
    }
    /**
    ...
    async sendEmail(options) {
        if (!this.enabled || !this.transporter) {
            core_1.logger.warn({ to: options.to }, 'Email service not enabled, skipping send');
            return false;
        }
        try {
            ...
            core_1.logger.info({ to: options.to, subject: options.subject }, '✉️ Email sent successfully');
            return true;
        }
        catch (error) {
            core_1.logger.error({ to: options.to, error: error.message }, '❌ Failed to send email');
            return false;
        }
    }
    /**
     ...
     */
    async sendDeletionOtpEmail(emailAddress, customerName, otp) {
        const html = `
     ...
     */
    async sendAdminDeletionOtpEmail(adminEmail, customerName, otp) {
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
     ...
     */
    async sendDeletionConfirmationEmail(emailAddress, customerName, recordCount) {
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
     ...
     */
    async sendPaymentReminderEmail(to, customerName, amount, shopName = 'Execora Shop') {
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
     ...
     */
    async sendInvoiceEmail(to, customerName, invoiceId, items, grandTotal, shopName = 'Execora Shop', pdfBuffer, pdfUrl, invoiceNo) {
     ...
     */
    async sendDailySummaryEmail(to, summary) {
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
