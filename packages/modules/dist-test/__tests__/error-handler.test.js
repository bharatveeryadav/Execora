...
(0, node_test_1.default)('AppError: default statusCode is 500 and category is UNKNOWN', () => {
    const err = new infrastructure_1.AppError('something went wrong');
    strict_1.default.equal(err.statusCode, 500);
    strict_1.default.equal(err.category, infrastructure_1.ErrorCategory.UNKNOWN);
    strict_1.default.equal(err.severity, infrastructure_1.ErrorSeverity.HIGH);
...
(0, node_test_1.default)('AppError: custom statusCode and category are preserved', () => {
    const err = new infrastructure_1.AppError('conflict', 409, infrastructure_1.ErrorCategory.CONFLICT, infrastructure_1.ErrorSeverity.MEDIUM);
    strict_1.default.equal(err.statusCode, 409);
    strict_1.default.equal(err.category, infrastructure_1.ErrorCategory.CONFLICT);
    strict_1.default.equal(err.severity, infrastructure_1.ErrorSeverity.MEDIUM);
});
// ── Subclass status codes ─────────────────────────────────────────────────────
(0, node_test_1.default)('ValidationError: statusCode is 400', () => {
    const err = new infrastructure_1.ValidationError('name required');
    strict_1.default.equal(err.statusCode, 400);
    strict_1.default.equal(err.name, 'ValidationError');
    strict_1.default.equal(err.category, infrastructure_1.ErrorCategory.VALIDATION);
    strict_1.default.equal(err.severity, infrastructure_1.ErrorSeverity.MEDIUM);
});
(0, node_test_1.default)('AuthenticationError: statusCode is 401', () => {
    const err = new infrastructure_1.AuthenticationError('not authenticated');
...
(0, node_test_1.default)('AuthorizationError: statusCode is 403', () => {
    const err = new infrastructure_1.AuthorizationError('forbidden');
...
(0, node_test_1.default)('NotFoundError: statusCode is 404 with resource message', () => {
    const err = new infrastructure_1.NotFoundError('Customer', 'cust-123');
    ...
    strict_1.default.equal(err.severity, infrastructure_1.ErrorSeverity.LOW);
});
(0, node_test_1.default)('NotFoundError: message without ID', () => {
    const err = new infrastructure_1.NotFoundError('Invoice');
    strict_1.default.equal(err.statusCode, 404);
...
(0, node_test_1.default)('DatabaseError: statusCode is 500', () => {
    ...
    const err = new infrastructure_1.DatabaseError('db failed', original);
    strict_1.default.equal(err.statusCode, 500);
    strict_1.default.equal(err.name, 'DatabaseError');
    strict_1.default.equal(err.category, infrastructure_1.ErrorCategory.DATABASE);
...
(0, node_test_1.default)('ExternalServiceError: statusCode is 502 and includes service name', () => {
    const err = new infrastructure_1.ExternalServiceError('WhatsApp', 'timeout');
...
(0, node_test_1.default)('BusinessLogicError: statusCode is 422', () => {
    const err = new infrastructure_1.BusinessLogicError('opening balance already set');
    strict_1.default.equal(err.statusCode, 422);
    strict_1.default.equal(err.name, 'BusinessLogicError');
    strict_1.default.equal(err.category, infrastructure_1.ErrorCategory.BUSINESS_LOGIC);
});
(0, node_test_1.default)('WebSocketError: statusCode is 500', () => {
    const err = new infrastructure_1.WebSocketError('ws dropped');
    strict_1.default.equal(err.statusCode, 500);
    strict_1.default.equal(err.name, 'WebSocketError');
    strict_1.default.equal(err.category, infrastructure_1.ErrorCategory.WEBSOCKET);
});
// ── ErrorHandler.formatErrorResponse ─────────────────────────────────────────
(0, node_test_1.default)('formatErrorResponse: AppError returns structured error object', () => {
    const err = new infrastructure_1.ValidationError('name required');
    const response = infrastructure_1.ErrorHandler.formatErrorResponse(err);
    ...
    strict_1.default.equal(response.error.category, infrastructure_1.ErrorCategory.VALIDATION);
...
(0, node_test_1.default)('formatErrorResponse: plain Error returns generic 500 response', () => {
    ...
    strict_1.default.equal(response.error.category, infrastructure_1.ErrorCategory.UNKNOWN);
});
...
(0, node_test_1.default)('formatWebSocketError: AppError returns type=error with correct message', () => {
    const err = new infrastructure_1.BusinessLogicError('balance insufficient');
    const ws = infrastructure_1.ErrorHandler.formatWebSocketError(err);
    ...
    strict_1.default.equal(ws.data.category, infrastructure_1.ErrorCategory.BUSINESS_LOGIC);
...
(0, node_test_1.default)('formatWebSocketError: plain Error uses WEBSOCKET category', () => {
    ...
    strict_1.default.equal(ws.data.category, infrastructure_1.ErrorCategory.WEBSOCKET);
});
// ── ErrorHandler.tryCatch ─────────────────────────────────────────────────────
(0, node_test_1.default)('tryCatch: success path returns { success: true, data }', async () => {
    const result = await infrastructure_1.ErrorHandler.tryCatch(async () => 42);
...
(0, node_test_1.default)('tryCatch: AppError path returns { success: false, error }', async () => {
    const result = await infrastructure_1.ErrorHandler.tryCatch(async () => {
        throw new infrastructure_1.ValidationError('bad input');
    });
    strict_1.default.equal(result.success, false);
    strict_1.default.ok(result.error instanceof infrastructure_1.AppError);
...
(0, node_test_1.default)('tryCatch: unknown error is wrapped as AppError', async () => {
    ...
    strict_1.default.equal(result.error?.category, infrastructure_1.ErrorCategory.UNKNOWN);
});
// ── ErrorHandler.handle ───────────────────────────────────────────────────────
(0, node_test_1.default)('handle: success path returns resolved value', async () => {
    const value = await infrastructure_1.ErrorHandler.handle(async () => 'hello');
    strict_1.default.equal(value, 'hello');
});
(0, node_test_1.default)('handle: AppError is re-thrown', async () => {
    await strict_1.default.rejects(() => infrastructure_1.ErrorHandler.handle(async () => { throw new infrastructure_1.NotFoundError('Customer', '123'); }), (err) => {
        strict_1.default.ok(err instanceof infrastructure_1.AppError);
        strict_1.default.equal(err.statusCode, 404);
        return true;
    });
});
(0, node_test_1.default)('handle: plain Error is wrapped and thrown as AppError', async () => {
    await strict_1.default.rejects(() => infrastructure_1.ErrorHandler.handle(async () => { throw new Error('db gone'); }), (err) => {
        strict_1.default.ok(err instanceof infrastructure_1.AppError);
        strict_1.default.equal(err.statusCode, 500);
        return true;
    });
