...
exports.responseTemplateService = void 0;
/**
...
class ResponseTemplateService {
     ...
     */
    generateFastResponse(intent, result) {
        if (!result.success) {
        ...
        switch (intent) {
            ...
            case 'LIST_REMINDERS':
                return `${data.count || 0} pending reminders hain${data.reminders?.length ? ` - ${data.reminders.map((r) => r.customer).join(', ')}` : ''}`;
            // CUSTOMERS
            ...
            case 'LIST_CUSTOMER_BALANCES': {
                ...
                const lines = data.customers.map((c) => `- ${c.name}: ₹${c.balance}${c.landmark ? ` (${c.landmark})` : ''}`);
    ...
    getErrorResponse(error) {
        const errorMap = {
            ...
            MULTIPLE_CUSTOMERS: 'Multiple customers found. Name se specifically batao (e.g., "Rahul ka...")',
...
exports.responseTemplateService = new ResponseTemplateService();
