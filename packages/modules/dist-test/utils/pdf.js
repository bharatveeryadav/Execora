...
exports.generateGstr1Pdf = generateGstr1Pdf;
exports.generatePnlPdf = generatePnlPdf;
...
async function generateInvoicePdf(data) {
    ...
    if (data.upiVpa) {
        try {
            const upiUrl = `upi://pay?pa=${encodeURIComponent(data.upiVpa)}` +
                `&pn=${encodeURIComponent(data.shopName)}` +
                `&am=${(data.grandTotal ?? data.subtotal).toFixed(2)}` +
                `&cu=INR`;
            qrBuffer = await qrcode_1.default.toBuffer(upiUrl, { width: 150, margin: 1, type: 'png' });
            core_1.logger.debug({ upiVpa: data.upiVpa }, 'UPI QR code generated for invoice PDF');
        }
        catch (err) {
            core_1.logger.warn({ err, upiVpa: data.upiVpa }, 'UPI QR generation failed — invoice PDF will render without QR');
        }
    }
    return new Promise((resolve, reject) => {
        try {
            ...
            if (hasTax) {
                doc.text(`Supply: ${isInterstate ? 'Inter-State (IGST)' : 'Intra-State (CGST + SGST)'}`, 315, invDetailY, { width: 220 });
                invDetailY += 12;
            ...
            data.items.forEach((item, idx) => {
                ...
                const gstPctLabel = item.gstRate !== undefined ? (item.gstRate === 0 ? 'Exempt' : `${item.gstRate}%`) : '-';
            ...
            if (hasTax) {
                ...
                const rows = [['Taxable Amount', data.subtotal, false]];
                ...
                const noTaxRows = [['Subtotal', data.subtotal]];
                if ((data.discountAmount ?? 0) > 0)
                    noTaxRows.push(['Discount', -(data.discountAmount ?? 0)]);
                if (data.roundOffAmount !== undefined && data.roundOffAmount !== 0)
                    noTaxRows.push(['Round Off', data.roundOffAmount]);
                ...
                noTaxRows.forEach(([label, value]) => {
                    ...
                    if (isTotal)
                        ...
                        doc.fillColor(isNeg ? '#dc2626' : '#374151').fontSize(9).font('Helvetica');
            ...
            if (hasTax && !isInterstate && (data.totalCgst ?? 0) > 0) {
                ...
                doc.fillColor('#6c757d')
                    .fontSize(7.5)
                    .font('Helvetica')
                    .text(`CGST @ ${(((data.totalCgst ?? 0) / data.subtotal) * 100).toFixed(1)}%  =  ${INR(data.totalCgst ?? 0)}   |   ` +
                    `SGST @ ${(((data.totalSgst ?? 0) / data.subtotal) * 100).toFixed(1)}%  =  ${INR(data.totalSgst ?? 0)}`, 62, y);
                y += 14;
            }
            // ── Amount in words ──────────────────────────────────────────────────────
            let wordsBoxH = 34;
            if (data.compositionScheme || data.reverseCharge)
                wordsBoxH += 20;
            doc.fillColor('#f8fafc')
            ...
            doc.fillColor('#94a3b8')
                .fontSize(7.5)
                .font('Helvetica')
                .text(`Generated by Execora  •  ${data.invoiceNo}  •  ${dateStr}  •  This is a computer-generated invoice.`, 50, 760, { align: 'center', width: 495 });
            doc.end();
            core_1.logger.debug({ invoiceNo: data.invoiceNo }, 'GST invoice PDF generated');
        }
        catch (err) {
            core_1.logger.error({ err }, 'PDF generation failed');
            reject(err);
        }
    });
}
// ── helpers shared by report PDFs ─────────────────────────────────────────────
const INR_FMT = (n) => `₹${n.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
function pdfTableHeader(doc, cols, y, rowH = 20) {
...
function pdfTableRow(doc, cols, y, odd, rowH = 16) {
    if (odd)
        doc.fillColor('#f8fafc').rect(46, y, 503, rowH).fill();
...
function pdfReportHeader(doc, title, subtitle, period) {
 ...
 */
function generateGstr1Pdf(data) {
    return new Promise((resolve, reject) => {
        try {
            const doc = new pdfkit_1.default({ margin: 46, size: 'A4', layout: 'landscape' });
            const chunks = [];
            ...
            if (data.b2b.length > 0) {
                ...
                const b2bCols = [
                    ...
                    { label: 'Invoice Value', x: 376, w: 65, align: 'right' },
                    { label: 'Taxable', x: 445, w: 52, align: 'right' },
                    { label: 'IGST', x: 501, w: 45, align: 'right' },
                ];
            ...
            if (data.b2cs.length > 0) {
                ...
                const b2csCols = [
                    ...
                    { label: 'GST Rate', x: 162, w: 50, align: 'center' },
                    { label: 'Taxable', x: 216, w: 80, align: 'right' },
                    { label: 'IGST', x: 300, w: 70, align: 'right' },
                    { label: 'CGST', x: 374, w: 70, align: 'right' },
                    { label: 'SGST', x: 448, w: 70, align: 'right' },
                ];
            ...
            if (data.hsn.length > 0) {
                ...
                const hsnCols = [
                    ...
                    { label: 'Qty', x: 243, w: 35, align: 'right' },
                    { label: 'GST%', x: 282, w: 30, align: 'center' },
                    { label: 'Taxable', x: 316, w: 65, align: 'right' },
                    { label: 'IGST', x: 385, w: 50, align: 'right' },
                    { label: 'CGST', x: 439, w: 50, align: 'right' },
                    { label: 'SGST', x: 493, w: 50, align: 'right' },
                ];
            ...
            doc.end();
            core_1.logger.debug({ invoiceCount: data.totals.invoiceCount }, 'GSTR-1 PDF generated');
        }
        catch (err) {
            core_1.logger.error({ err }, 'GSTR-1 PDF generation failed');
            reject(err);
        }
    });
}
 ...
 */
function generatePnlPdf(data) {
    return new Promise((resolve, reject) => {
        try {
            const doc = new pdfkit_1.default({ margin: 46, size: 'A4' });
            const chunks = [];
            doc.on('data', (c) => chunks.push(c));
            doc.on('end', () => resolve(Buffer.concat(chunks)));
            doc.on('error', reject);
            ...
            const pnlCols = [
                ...
                { label: 'Bills', x: 114, w: 28, align: 'center' },
                { label: 'Revenue', x: 146, w: 68, align: 'right' },
                { label: 'Discounts', x: 218, w: 62, align: 'right' },
                { label: 'Net Revenue', x: 284, w: 68, align: 'right' },
                { label: 'Tax', x: 356, w: 56, align: 'right' },
                { label: 'Collected', x: 416, w: 62, align: 'right' },
                { label: 'Outstanding', x: 482, w: 60, align: 'right' },
            ];
            ...
            const totRows = [
                ...
                { value: String(t.invoiceCount), x: 114, w: 28, align: 'center' },
                { value: INR_FMT(t.revenue), x: 146, w: 68, align: 'right' },
                { value: INR_FMT(t.discounts), x: 218, w: 62, align: 'right' },
                { value: INR_FMT(t.netRevenue), x: 284, w: 68, align: 'right' },
                { value: INR_FMT(t.taxCollected), x: 356, w: 56, align: 'right' },
                { value: INR_FMT(t.collected), x: 416, w: 62, align: 'right' },
                { value: INR_FMT(t.outstanding), x: 482, w: 60, align: 'right' },
            ];
            ...
            doc.end();
            core_1.logger.debug({ months: data.months.length }, 'P&L PDF generated');
        }
        catch (err) {
            core_1.logger.error({ err }, 'P&L PDF generation failed');
            reject(err);
        }
    });
}
