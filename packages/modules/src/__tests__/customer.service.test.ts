...
test('calculateBalanceFromLedger: returns invoiced amount minus payments', async () => {
  const restores: RestoreFn[] = [];
  try {
    restores.push(patchMethod(prisma.invoice as any, 'aggregate', async () => ({
      _sum: { total: dec(1500) },
    })));
    restores.push(patchMethod(prisma.payment as any, 'aggregate', async () => ({
      _sum: { amount: dec(300) },
    })));
...
test('calculateBalanceFromLedger: returns 0 when no invoices or payments', async () => {
  const restores: RestoreFn[] = [];
  try {
    restores.push(patchMethod(prisma.invoice as any, 'aggregate', async () => ({
      _sum: { total: null },
