...
import type { AgingReport, AgingBucket, AgingCustomerRow } from "./types";

const BUCKETS: Array<{ label: string; minDays: number; maxDays: number }> = [
...
export async function getAgingReport(
  ...
  const bucketTotals: number[] = [0, 0, 0, 0];
  ...
  for (const inv of invoices) {
    ...
    if (bi === 0) row.bucket0to30 = round2(row.bucket0to30 + due);
    if (bi === 1) row.bucket31to60 = round2(row.bucket31to60 + due);
    if (bi === 2) row.bucket61to90 = round2(row.bucket61to90 + due);
    if (bi === 3) row.bucket91plus = round2(row.bucket91plus + due);
  }
  ...
  const buckets: AgingBucket[] = BUCKETS.map((b, i) => ({
    ...
    count: customers.filter((c) => {
      if (i === 0) return c.bucket0to30 > 0;
      if (i === 1) return c.bucket31to60 > 0;
      if (i === 2) return c.bucket61to90 > 0;
