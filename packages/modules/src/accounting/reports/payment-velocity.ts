...
import type { PaymentVelocityReport, PaymentVelocityRow } from "./types";
 ...
 */
export async function getPaymentVelocityReport(
  tenantId: string,
  from: Date,
  to: Date,
): Promise<PaymentVelocityReport> {
  ...
  type Acc = {
    ...
    daysSumPaid: number; // sum of days-to-pay for paid invoices
    paidWithKnownDate: number; // denominator for avg
  };

  const customerMap = new Map<string, Acc>();
  ...
  const rows: PaymentVelocityRow[] = [...customerMap.values()].map((acc) => ({
    customerId: acc.customerId,
    customerName: acc.customerName,
    phone: acc.phone,
    avgDaysToPay:
      acc.paidWithKnownDate > 0
