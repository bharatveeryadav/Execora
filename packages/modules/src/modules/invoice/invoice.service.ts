...
class InvoiceService {
	 ...
	 */
	private async findOrCreateProduct(tx: any, productName: string): Promise<{ product: any; autoCreated: boolean }> {
		const norm = (s: string) =>
			s
				.toLowerCase()
				.replace(/[^a-z0-9]/g, '')
	...
	async dispatchInvoicePdfEmail(invoiceId: string): Promise<void> {
		...
		try {
			...
			const shopName = settings.shopName || process.env.SHOP_NAME || tenant?.name || 'Execora Shop';
