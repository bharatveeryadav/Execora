...
class CustomerService {
	 ...
	 */
	async getActiveCustomerById(customerId: string, conversationId: string): Promise<CustomerSearchResult | null> {
		const customer = await prisma.customer.findUnique({
			where: { id: customerId },
	 ...
	 */
	async searchCustomerWithContext(query: string, conversationId: string): Promise<CustomerSearchResult[]> {
		const context = this.getContext(conversationId);
		...
		if (this.isCacheValid(context.timestamp) && context.recentCustomers.length > 0) {
			...
			if (cachedResults.length > 0) {
				logger.info({ conversationId, cacheHits: cachedResults.length }, '⚡ Customer search from cache');
	 ...
	 */
	async getMultipleCustomersWithContext(customerIds: string[], conversationId: string) {
		const context = this.getContext(conversationId);
		...
		const searchResults: CustomerSearchResult[] = customers.filter(Boolean).map((c: any) => ({
			id: c.id,
			name: c.name,
			phone: c.phone,
			nickname: Array.isArray(c.nickname) ? (c.nickname[0] ?? null) : c.nickname,
	 ...
	 */
	async prefetchConversationContext(conversationId: string, latestCustomerId?: string) {
		const context = this.getContext(conversationId);
	 ...
	 */
	async updateCustomerInstant(customerId: string, updates: CustomerUpdateData, conversationId: string) {
		try {
	 ...
	 */
	async searchCustomerRanked(query: string, conversationId: string): Promise<CustomerSearchResult[]> {
	...
	async resolveCustomerAmbiguity(
		...
		logger.info({ conversationId, candidates: similar.length }, '🤔 Multiple candidates found');
	 ...
	 */
	async confirmCustomerSelection(customerId: string, conversationId: string, updateFields?: CustomerUpdateData) {
	 ...
	 * Create new customer
	 */
	async createCustomer(data: { name: string; phone?: string; nickname?: string; landmark?: string; notes?: string }) {
		try {
			...
			if (existing.length > 0) {
				logger.warn(
					...
					'🚫 Duplicate customer name - creation blocked'
				);
	...
	async deleteCustomerAndAllData(customerId: string): Promise<{
		...
		try {
			...
			logger.error({ customerId, error: error.message }, 'Failed to delete customer and all related data');
			return {
				success: false,
				error: error.message,
				deletedRecords: {
	...
	async updateProfile(
		...
		logger.info({ customerId: id, fields: Object.keys(updateData) }, 'Customer profile updated');
		return updated;
	}
	...
	async getCommPrefs(customerId: string) {
		return prisma.customerCommunicationPrefs.findUnique({
			where: { customerId },
		});
	}

	async upsertCommPrefs(
		customerId: string,
		data: {
		...
		const upserted = await prisma.customerCommunicationPrefs.upsert({
			where: { customerId },
			create: {
		...
		logger.info({ customerId, fields: Object.keys(data) }, 'CommPrefs upserted');
		return upserted;
	}
}
