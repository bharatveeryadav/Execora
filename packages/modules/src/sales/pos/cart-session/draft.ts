    ...
    case "purchase_entry": {
      const expense = await prisma.expense.create({
        data: {
          ...
          quantity:
            data.quantity != null
              ? new Prisma.Decimal(data.quantity as number)
              : null,
          unit: (data.unit as string) ?? null,
          ratePerUnit:
            data.ratePerUnit != null
              ? new Prisma.Decimal(data.ratePerUnit as number)
              : null,
    ...
    case "product": {
      const product = await prisma.product.create({
        data: {
          ...
          cost:
            data.cost != null ? new Prisma.Decimal(data.cost as number) : null,
          wholesalePrice:
            data.wholesalePrice != null
              ? new Prisma.Decimal(data.wholesalePrice as number)
              : null,
          priceTier2:
            data.priceTier2 != null
              ? new Prisma.Decimal(data.priceTier2 as number)
              : null,
          priceTier3:
            data.priceTier3 != null
              ? new Prisma.Decimal(data.priceTier3 as number)
              : null,
          ...
          gstRate:
            data.gstRate != null
              ? new Prisma.Decimal(data.gstRate as number)
