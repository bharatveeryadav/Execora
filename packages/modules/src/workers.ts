...
const reminderWorker = new Worker<ReminderJobData>(
  ...
  async (job) => {
    ...
    const { reminderId, customerId, customerName, phone, amount, message } =
      job.data;
    logger.info(
      {
        queue: "reminders",
    ...
    if (!delivered && phone) {
      ...
      if (waResult.success) {
        ...
        logger.warn(
          { reminderId, phone, error: waResult.error },
          "WhatsApp reminder delivery failed",
        );
      }
    }
    ...
    if (delivered && deliveryChannel) {
      try {
        ...
        logger.error(
          { dbError, reminderId },
          "Failed to create message log (message was sent)",
        );
      }
    }
    ...
    if (delivered) {
      ...
      queueJobDuration.observe(
        { queue: "reminders", status: "success" },
...
reminderWorker.on("completed", (job) => {
  logger.info(
    { queue: "reminders", jobId: job.id, reminderId: job.data.reminderId },
...
const whatsappWorker = new Worker<WhatsAppJobData>(
  ...
  async (job) => {
    ...
    logger.info(
      { queue: "whatsapp", jobId: job.id, phone },
    ...
    if (result.success) {
      try {
        ...
        logger.error(
          { dbError, phone },
          "Failed to create WhatsApp message log (message was sent)",
        );
      }

      queueJobsProcessed.inc({ queue: "whatsapp", status: "success" });
      queueJobDuration.observe(
        { queue: "whatsapp", status: "success" },
        (Date.now() - jobStart) / 1000,
      );
      logger.info(
        { queue: "whatsapp", messageId: result.messageId },
...
const ocrWorker = new Worker<OcrJobData>(
  ...
  async (job) => {
    ...
    const ocrJobRow = await prisma.ocrJob
      .update({
        where: { id: ocrJobId },
...
Extract ALL line items visible. Use Indian currency (INR). Omit unclear fields.`;
        ...
        try {
          ...
          throw new Error(
            "OpenAI returned non-JSON response for purchase bill OCR",
          );
        }
        ...
        for (const item of items) {
          ...
          if (!product) {
            log.warn(
              { itemName: item.name },
              "OCR: no matching product found for purchase item, skipping PO line",
            );
            continue;
          }
        ...
        log.info(
          { poId: po.id, itemCount: items.length },
          "OCR purchase_bill job completed",
        );
...
- Do NOT include markdown, comments, or explanation — pure JSON only.`;
        ...
        log.info(
          { created },
          "OCR product_catalog job completed — drafts created, awaiting review",
        );
...
ocrWorker.on("failed", (job, err) => {
  ...
  logger.error(
    { queue: "ocr-jobs", jobId: job?.id, error: err.message },
    "OCR job failed permanently",
  );
});
...
const startWorkerMonitoring = () => {
  ...
  const runSnapshot = async () => {
    try {
      const [reminderCounts, whatsappCounts, ocrCounts, redisOk] =
        await Promise.all([
      ...
      logger.error(
        { error: error?.message },
        "Worker monitoring snapshot failed",
      );
    }
  };
...
const reactionWorker = new Worker<DomainEvent<unknown>>(
  "domain-events",
  async (job) => {
    const jobStart = Date.now();
    const event = job.data as DomainEvent<unknown>;
    logger.info(
      {
        queue: "domain-events",
        jobId: job.id,
    ...
    try {
      await eventBus.react(event);
      queueJobsProcessed.inc({ queue: "domain-events", status: "success" });
      queueJobDuration.observe(
        { queue: "domain-events", status: "success" },
        (Date.now() - jobStart) / 1000,
      );
    } catch (err) {
      queueJobsProcessed.inc({ queue: "domain-events", status: "failed" });
      queueJobDuration.observe(
        { queue: "domain-events", status: "failed" },
        (Date.now() - jobStart) / 1000,
      );
      errorCounter.inc({
        service: "worker",
      ...
      logger.error(
        { err, eventName: event.name, jobId: job.id },
        "Domain event handler threw",
      );
...
reactionWorker.on("completed", (job) => {
  logger.debug(
    { queue: "domain-events", jobId: job.id, eventName: job.name },
