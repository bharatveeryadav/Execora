...
// Sprint 2: OCR / AI image-processing jobs
export const ocrJobQueue = new Queue("ocr-jobs", {
  connection,
  defaultJobOptions: {
    attempts: 2,
...
// ── Domain events queue ───────────────────────────────────────────────────────
// Single BullMQ queue for all cross-module domain events.
// Job name = DomainEventName (e.g. "InvoiceCreated").
// Job data = DomainEvent<T> envelope.
// Workers that subscribe to specific event names simply check job.name.
...
export const ocrJobQueueEvents = new QueueEvents("ocr-jobs", { connection });
export const eventsQueueEvents = new QueueEvents("domain-events", {
  connection,
});
...
export const closeQueues = async () => {
  ...
  await ocrJobQueue.close();
  await eventsQueue.close();
  await reminderQueueEvents.close();
  await whatsappQueueEvents.close();
  await ocrJobQueueEvents.close();
  await eventsQueueEvents.close();
