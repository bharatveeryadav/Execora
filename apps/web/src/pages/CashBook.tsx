	...
	return (
		<div className="min-h-screen bg-background pb-24">
			...
			<main className="mx-auto max-w-2xl space-y-4 p-4">
				{/* Balance card */}
				<Card
					className={`border-none shadow-md ${balance >= 0 ? 'bg-green-50 dark:bg-green-950/20' : 'bg-red-50 dark:bg-red-950/20'}`}
				>
					<CardContent className="p-5 text-center">
						...
						<p
							className={`text-4xl font-extrabold tabular-nums ${balance >= 0 ? 'text-green-700 dark:text-green-400' : 'text-destructive'}`}
						>
							{balance >= 0 ? '+' : '-'}
							{fmt(balance)}
						</p>
				...
				{isLoading ? (
					...
					<div className="space-y-2">
						{entries.map((txn) => (
							<div key={txn.id} className="flex items-center gap-3 rounded-xl border bg-card px-4 py-3">
								<div
								...
								<p
									className={`shrink-0 font-bold tabular-nums ${txn.type === 'in' ? 'text-green-700 dark:text-green-400' : 'text-destructive'}`}
								>
									{txn.type === 'in' ? '+' : '-'}
									{fmt(txn.amount)}
								</p>
