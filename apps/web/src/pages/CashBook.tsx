    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <header className="sticky top-0 z-30 border-b bg-card px-4 py-3">
        ...
        <VoiceBar
          idleHint={
            <>
              <span className="font-medium">"Aaj ka cash dikhao"</span> ·{" "}
              <span>"Is mahine ka balance?"</span>
            </>
          }
        />
      </header>
      ...
      <main className="mx-auto max-w-2xl space-y-4 p-4">
        {/* Balance card */}
        <Card
          className={`border-none shadow-md ${balance >= 0 ? "bg-success/10" : "bg-destructive/10"}`}
        >
          <CardContent className="p-5 text-center">
            ...
            <p
              className={`text-4xl font-extrabold tabular-nums ${balance >= 0 ? "text-success" : "text-destructive"}`}
            >
              {balance >= 0 ? "+" : "-"}
              {fmt(balance)}
            </p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Card className="border-none shadow-sm">
            <CardContent className="flex items-center gap-3 p-4">
              <ArrowUpRight className="h-8 w-8 rounded-full bg-success/10 p-1.5 text-success" />
              <div>
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
                  Today In
                </p>
                <p className="text-xl font-bold text-success">{fmt(todayIn)}</p>
              </div>
            </CardContent>
          </Card>
          <Card className="border-none shadow-sm">
            <CardContent className="flex items-center gap-3 p-4">
              <ArrowDownLeft className="h-8 w-8 rounded-full bg-destructive/10 p-1.5 text-destructive" />
              <div>
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
                  Today Out
                </p>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed py-12 text-center">
            ...
            <p className="text-xs text-muted-foreground">
              Collect payments or add expenses to see entries here
            </p>
          ...
          <div className="space-y-2">
            {entries.map((txn) => (
              ...
              >
                ...
                <p
                  className={`shrink-0 font-bold tabular-nums ${txn.type === "in" ? "text-success" : "text-destructive"}`}
                >
                  {txn.type === "in" ? "+" : "-"}
                  {fmt(txn.amount)}
                </p>
