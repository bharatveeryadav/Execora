...
export default function InvoiceDetail() {
	...
	function editChangeDiscount(idx: number, pct: number) {
		setEditItems((prev) =>
			prev.map((it, i) => (i === idx ? { ...it, discount: Math.min(100, Math.max(0, pct || 0)) } : it))
		);
	}
	...
	const waLink = customerPhone ? `https://wa.me/91${customerPhone.replace(/\D/g, '')}?text=${waText}` : null;
				...
				<div className="grid grid-cols-2 gap-3">
					{waLink && (
						<Button
							asChild
							variant="outline"
							className="gap-2 border-green-500/30 text-green-700 hover:bg-green-50 dark:text-green-400 dark:hover:bg-green-900/20"
						>
				...
				<DialogContent className="sm:max-w-sm">
					...
					<div className="space-y-4 py-2">
						<p className="text-sm text-muted-foreground">
							This will convert the proforma into a final tax invoice. Optionally record an initial
							payment.
						</p>
							...
							<table className="w-full">
								...
								<tbody>
									{editItems.map((item, idx) => {
										const lineTotal =
											Math.round(item.price * item.qty * (1 - (item.discount || 0) / 100) * 100) /
											100;
										return (
