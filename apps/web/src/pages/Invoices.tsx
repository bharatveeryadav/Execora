...
const Invoices = () => {
	...
	const totalValue = filtered.reduce((s, inv) => s + parseFloat(String(inv.total ?? 0)), 0);
				...
				<div className="mx-auto max-w-3xl overflow-x-auto px-4">
					<div className="flex gap-0 border-t">
						{STATUS_TABS.map((tab) => {
							...
							const count = tab === 'All' ? invoices.length : (counts[key] ?? 0);
							return (
								<button
									key={tab}
					...
					{isLoading ? (
						...
						<div className="flex flex-col items-center gap-3 py-12 text-center">
							...
							<p className="text-sm text-muted-foreground">
								{search || statusTab !== 'All' ? 'No invoices match your filter' : 'No invoices yet'}
							</p>
						...
						<div className="divide-y">
							{filtered.map((inv: Invoice) => {
								const amtColor = AMOUNT_COLORS[inv.status] ?? AMOUNT_COLORS.default;
								return (
									<div
										key={inv.id}
									...
									>
										...
										<div className="min-w-0 flex-1">
											<p className="truncate text-sm font-semibold">{inv.invoiceNo}</p>
											<p className="truncate text-xs text-muted-foreground">
												{inv.customer?.name ?? 'Unknown'} · {formatDate(inv.createdAt)}
											</p>
										</div>
