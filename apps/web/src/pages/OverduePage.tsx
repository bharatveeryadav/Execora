...
function agingLabel(days: number | null): {
  ...
  if (!days || days <= 0)
    return { label: "—", cls: "text-muted-foreground", icon: "⚪" };
  if (days >= 30)
    return {
      label: `${days}d`,
  ...
  if (days >= 7)
    return { label: `${days}d`, cls: "text-warning font-medium", icon: "🟡" };
...
function agingBadgeCls(days: number | null) {
  ...
  if (days >= 7) return "bg-warning/15 text-warning border-warning/30";
...
function daysLeftBadgeCls(days: number) {
  if (days <= 1)
    return "bg-destructive/15 text-destructive border-destructive/30";
  if (days <= 5) return "bg-warning/15 text-warning border-warning/30";
...
function computeScheduleDate(when: WhenOption): string {
  ...
  const days =
    when === "today" ? 0 : when === "tomorrow" ? 1 : parseInt(when, 10);
...
function RecordPaymentDialog({ customer, onClose }: PayDialogProps) {
  ...
  const [amount, setAmount] = useState(
    String(Math.max(0, parseFloat(String(customer.balance)))),
  );
  ...
  const submit = async () => {
    ...
    try {
      ...
      fireConfetti();
      toast({
        title: `💰 ₹${amt.toLocaleString("en-IN")} recorded for ${customer.name}`,
      });
      onClose();
    } catch (e: unknown) {
      toast({
...
function ScheduleReminderDialog({
  ...
  const [amount, setAmount] = useState(
    prefillCustomer
      ? String(Math.max(0, parseFloat(String(prefillCustomer.balance))))
      : "",
  );
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <Select
              value={when}
              onValueChange={(v) => setWhen(v as WhenOption)}
            >
              <SelectTrigger className="mt-1">
                <SelectValue />
              </SelectTrigger>
...
const OverduePage = () => {
  ...
  const getDaysOverdue = useMemo(() => {
    ...
    for (const c of allCustomers) {
      ...
      const oldest = unpaid.reduce((prev, cur) =>
        new Date(cur.dueDate ?? cur.createdAt) <
        new Date(prev.dueDate ?? prev.createdAt)
          ? cur
          : prev,
      );
      const days = Math.ceil(
        (Date.now() - new Date(oldest.dueDate ?? oldest.createdAt).getTime()) /
          86_400_000,
      );
  ...
  const overdueCustomers = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      if (sort === "amount")
        return parseFloat(String(b.balance)) - parseFloat(String(a.balance));
      if (sort === "days") {
  ...
  const getDaysLeft = (t: string) =>
    Math.max(0, Math.ceil((new Date(t).getTime() - Date.now()) / 86_400_000));
  ...
  const sendBulkReminders = async (ids: string[]) => {
    try {
      ...
      const count =
        (result as { reminders?: unknown[] }).reminders?.length ?? 0;
      toast({
      ...
      <div className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur">
        ...
        <div className="mx-auto flex max-w-3xl border-t px-4">
          ...
          >
            ...
            Overdue
            {criticalCount > 0 && (
              <span className="ml-0.5 rounded-full bg-destructive px-1.5 py-0.5 text-[10px] font-bold text-destructive-foreground leading-none">
                {criticalCount}
              </span>
            )}
          </button>
          ...
          <>
            ...
            {overdueCustomers.length > 0 && (
              <div className="flex items-center gap-3 rounded-lg border bg-muted/30 px-3 py-2">
                <Checkbox
                  checked={
                    selected.size === overdueCustomers.length &&
                    overdueCustomers.length > 0
                  }
                ...
                {selected.size > 0 && (
                  <div className="flex gap-1">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        exportCSV(
                          overdueCustomers.filter((c) => selected.has(c.id)),
                        )
                      }
                    >
            ...
            {overdueCustomers.length > 0 && (
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={bulkReminders.isPending}
                  onClick={() =>
                    sendBulkReminders(overdueCustomers.map((c) => c.id))
                  }
                >
