...
const TYPE_CFG: Record<
  TxType,
  { emoji: string; label: string; bg: string; textCls: string }
> = {
  invoice: {
    emoji: "🧾",
    label: "Invoice",
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <main className="mx-auto max-w-3xl space-y-4 p-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Card
            className={`border-none shadow-sm ${net >= 0 ? "bg-success/5" : "bg-destructive/5"}`}
          >
            <CardContent className="p-3 text-center">
              <Calendar className="mx-auto mb-1 h-4 w-4 text-muted-foreground" />
              <p
                className={`text-sm font-bold tabular-nums ${net >= 0 ? "text-success" : "text-destructive"}`}
              >
                {net >= 0 ? "+" : ""}
                {formatCurrency(net)}
              </p>
        ...
        {!isLoading && filtered.length === 0 && (
          <div className="flex flex-col items-center gap-3 py-16 text-center">
            ...
            <p className="text-xs text-muted-foreground">
              Create invoices, record payments, or add expenses to see them
              here.
            </p>
          </div>
        )}
