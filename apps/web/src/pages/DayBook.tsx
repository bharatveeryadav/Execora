...
const TYPE_CFG: Record<TxType, { emoji: string; label: string; bg: string; textCls: string }> = {
  ...
  cash_in:   { emoji: "💵", label: "Cash In",    bg: "bg-emerald-500/10",  textCls: "text-emerald-700 dark:text-emerald-400" },
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{filtered.length} entries · {period}</p>
              </div>
            </div>
      ...
      <main className="mx-auto max-w-3xl space-y-4 p-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <Card className="border-none shadow-sm">
            <CardContent className="p-3 text-center">
              ...
              <p className="text-sm font-bold text-green-700 dark:text-green-400 tabular-nums">{formatCurrency(totalIn)}</p>
          ...
          <Card className="border-none shadow-sm">
            <CardContent className="p-3 text-center">
              ...
              <p className="text-sm font-bold text-destructive tabular-nums">{formatCurrency(totalOut)}</p>
          ...
          <Card className={`border-none shadow-sm ${net >= 0 ? "bg-green-500/5" : "bg-destructive/5"}`}>
            <CardContent className="p-3 text-center">
              ...
              <p className={`text-sm font-bold tabular-nums ${net >= 0 ? "text-green-700 dark:text-green-400" : "text-destructive"}`}>
        ...
        {!isLoading && filtered.length === 0 && (
          <div className="flex flex-col items-center gap-3 py-16 text-center">
            ...
            <p className="text-xs text-muted-foreground">Create invoices, record payments, or add expenses to see them here.</p>
          </div>
        )}
                ...
                <div className="divide-y">
                  {txs.map((tx) => {
                    ...
                    return (
                      ...
                      >
                        {/* Icon */}
                        <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${cfg.bg} text-base`}>
                          {cfg.emoji}
                        </div>
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {tx.sublabel && (
                            <p className="truncate text-xs text-muted-foreground">{tx.sublabel}</p>
                          )}
                        </div>

                        {/* Amount */}
                        <p className={`shrink-0 text-sm font-bold tabular-nums ${
                          tx.sign === "credit" ? "text-green-700 dark:text-green-400" : "text-foreground"
                        }`}>
                          {tx.sign === "credit" ? "+" : "−"}{formatCurrency(tx.amount)}
                        </p>
                      </div>
                    );
                  })}
