            ...
            <CardContent className="flex items-center justify-between p-3">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="text-sm">
                  Total spend ({filterTab.toLowerCase()})
                </span>
              </div>
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-sm">
          ...
          <form onSubmit={handleAdd} className="space-y-3 py-1">
            ...
            <div className="space-y-1">
              ...
              <Select
                value={category}
                onValueChange={(v) => setCategory(v as PurchaseCategory)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <DialogFooter>
              <Button
                type="submit"
                disabled={
                  !itemName.trim() || !rate || !qty || createPurchase.isPending
                }
              >
            ...
            <div className="space-y-4 py-2">
              <div className="flex items-center gap-2 text-success">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {Array.isArray(ocrJob.parsedItems)
                      ? ocrJob.parsedItems.length
                      : 0}{" "}
                    items extracted
                  </p>
                </div>
              ...
              {ocrJob.purchaseOrderId && (
                <div className="rounded-lg bg-success/10 px-3 py-2 text-sm text-success">
                  ✅ Purchase order created &amp; stock levels updated
                  automatically.
                </div>
              )}
              ...
              {Array.isArray(ocrJob.parsedItems) &&
                ocrJob.parsedItems.length > 0 && (
                  <div className="max-h-48 space-y-1 overflow-y-auto rounded-md border p-2">
                    {(
                      ocrJob.parsedItems as Array<{
                        name?: string;
                        qty?: number;
                        rate?: number;
                      }>
