...
const VoiceBar = ({ idleHint = DEFAULT_HINT }: VoiceBarProps) => {
	...
	const startSpeechRecognitionRef = useRef<() => void>(() => {
		// assigned after callback declaration
	});
	...
	const speakWithBrowser = useCallback((text: string) => {
		...
		const selectedProvider = localStorage.getItem('execora:ttsProvider') ?? ttsProviderRef.current;
		...
		utterance.lang = navigator.language?.startsWith('hi') ? 'hi-IN' : navigator.language || 'en-IN';
		utterance.rate = 1;
		utterance.pitch = 1;
	...
	const ensurePlaybackReady = useCallback(async () => {
		...
		const AudioContextCtor = (window.AudioContext ||
			(window as { webkitAudioContext?: typeof AudioContext }).webkitAudioContext) as
			| (new () => AudioContext)
			| undefined;
	...
	const playTtsAudio = useCallback(async (audioBase64: string, format = 'mp3') => {
		...
		const offs = [
			...
			wsClient.on('voice:tts-stream', (payload) => {
				...
				if (audioData) {
					...
					void playTtsAudio(audioData, fmt).catch(() => {
						if (lastResponseTextRef.current) {
							speakWithBrowser(lastResponseTextRef.current);
						}
					});
				}
			}),
	...
	const startVoice = useCallback(async () => {
		...
		if (!navigator.mediaDevices?.getUserMedia) {
			...
			return startSpeechRecognitionRef.current();
		}
		...
		try {
			...
			return startSpeechRecognitionRef.current(); // mic denied → browser SR fallback
		}
	...
	return (
		<div className="mt-3 flex items-center gap-3 rounded-lg bg-muted px-4 py-2.5">
			...
			<p className="min-w-0 flex-1 truncate text-sm text-muted-foreground">{liveText ?? idleHint}</p>
		</div>
	);
};
