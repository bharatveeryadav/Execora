    ...
    <div className="grid grid-cols-2 gap-3">
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) =>
            set("quantity", parseFloat(e.target.value) || undefined)
          }
          placeholder="1"
        />
      </div>
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) =>
            set("ratePerUnit", parseFloat(e.target.value) || undefined)
          }
          placeholder="0.00"
        />
      </div>
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <div className="col-span-2">
        <Label>Product</Label>
        <Input
          value={String(data.productName ?? data.productId ?? "")}
          readOnly
          className="bg-muted"
        />
      </div>
...
export function DraftConfirmDialog({
  ...
  const confirmMutation = useMutation({
    mutationFn: async () => {
      ...
      await draftApi.update(draft!.id, {
        data: localData,
        notes: notes || undefined,
      });
    ...
    onSuccess: ({ draft: confirmed }) => {
      toast({
        ...
        description:
          confirmed.title ?? TYPE_LABEL[confirmed.type as Draft["type"]],
      });
      qc.invalidateQueries({ queryKey: ["drafts"] });
