...
function loadPriceTiers(): typeof DEFAULT_PRICE_TIERS {
  try {
    return (
      JSON.parse(localStorage.getItem("execora:pricetiers") ?? "null") ??
      DEFAULT_PRICE_TIERS
    );
  } catch {
...
const InvoiceCreation = ({
    ...
    try {
      ...
      if (res?.product) {
        ...
        setItems((prev) => {
          ...
          return [
            ...prev,
            { name: p.name, qty: "1", price, discount: 0, total: price },
          ];
        });
      }
    } catch {
        ...
        if (resolvedItems?.length) {
          setItems(
            resolvedItems.map((it) => {
              ...
              const total =
                pct > 0
                  ? Math.round(price * qty * (1 - pct / 100) * 100) / 100
                  : baseTotal;
              return {
                name: it.productName,
                qty: String(qty),
  ...
  const startVoiceInput = useCallback(async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      ...
      const rec = new SR();
      rec.lang = navigator.language?.startsWith("hi")
        ? "hi-IN"
      ...
      rec.onresult = (e: unknown) => {
        const results = (
          e as { results?: ArrayLike<ArrayLike<{ transcript?: string }>> }
        )?.results;
    ...
    recorder.ondataavailable = (e: BlobEvent) => {
      if (e.data.size > 0 && wsClient.isConnected && canStreamAudioRef.current)
        wsClient.sendBinary(e.data);
    };
    recorder.onstop = () => {
      canStreamAudioRef.current = false;
      setIsListening(false);
  ...
  const handleSelectCustomer = async (c: Customer) => {
    ...
    const opts = {
      ...
      discountAmount:
        discountPct === 0 && discountFlat > 0 ? discountFlat : undefined,
      buyerGstin: buyerGstin.trim() || undefined,
      placeOfSupply: placeOfSupply.trim() || undefined,
      initialPayment:
        partialAmount > 0
          ? { amount: partialAmount, method: partialMethod }
          : undefined,
    };
  ...
  const handlePrintInvoice = () => {
    ...
    const invoiceLabel =
      createdInvoiceNo || (isProforma ? "PRO-DRAFT" : "INV-DRAFT");
  ...
  const handleWalkInCustomer = async () => {
    ...
    try {
      ...
      const existing = found.find((c: Customer) =>
      ...
      if (/duplicate|already exists|unique/i.test(msg)) {
        try {
          ...
          const found2 = retry.find((c: Customer) =>
            /walk\s*-?\s*in|cash\s*customer/i.test(c.name),
          );
          if (found2) {
  ...
  const handleInlineConfirm = async () => {
    ...
    const dominantMethod: "cash" | "upi" | "card" | "other" =
      splitUpi >= splitCash && splitUpi >= splitCard
        ? "upi"
        : splitCard > splitCash
          ? "card"
          : "cash";
    ...
    const opts = {
      ...
      initialPayment:
        splitTotal > 0
          ? { amount: splitTotal, method: dominantMethod }
          : undefined,
    };
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-lg">
              🧾{" "}
              {step === "start" &&
                (isProforma ? "New Quotation / Proforma" : "New Invoice")}
              {step === "listening" && "Listening..."}
              {step === "processing" && "Processing..."}
              {step === "confirmation" && "Confirm Items"}
              {step === "customer" && "Add Customer"}
              {step === "final" &&
                `${isProforma ? "Proforma" : "Invoice"} ${createdInvoiceNo || "#NEW"}`}
            ...
            <div className="flex flex-col gap-5 py-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                {withGst && (
                  <ToggleChip
                    ...
                    onClick={() =>
                      setSupplyType((s) =>
                        s === "INTRASTATE" ? "INTERSTATE" : "INTRASTATE",
                      )
                    }
                    icon="🚛"
                    label={
                ...
                <div className="flex gap-1 flex-wrap">
                  {priceTiers.map((tier, idx) => (
                    ...
                    >
                      {tier.name}
                      {tier.disc > 0 && (
                        <span className="opacity-60"> -{tier.disc}%</span>
                      )}
                    </button>
                  ))}
                </div>
              ...
              {!isProforma && (
                <div className="flex items-center gap-2">
                  ...
                  <select
                    value={partialMethod}
                    onChange={(e) =>
                      setPartialMethod(e.target.value as typeof partialMethod)
                    }
              ...
              <div className="flex flex-col items-center gap-3 pt-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    setItems([
                      { name: "", qty: "1", price: 0, discount: 0, total: 0 },
                    ]);
                    setStep("manual");
                  }}
                >
              ...
              {!isProforma && (
                <div className="rounded-lg border bg-muted/20 px-3 py-2.5 space-y-2">
                  <p className="flex items-center gap-1.5 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                    <Wallet className="h-3 w-3" /> Payment received now
                    (optional)
                  </p>
                        ...
                        splitCash + splitUpi + splitCard < total && (
                          <span className="ml-1 text-muted-foreground">
                            ...
                            {formatCurrency(
                              Math.max(
                                0,
                                total - splitCash - splitUpi - splitCard,
                              ),
                            )}
                          </span>
                        )}
              ...
              {partialAmount > 0 && (
                <div className="rounded-lg bg-green-50 px-3 py-2 text-xs dark:bg-green-950/30">
                  <span className="text-green-700 dark:text-green-400 font-medium">
                    ✓ Paid: {formatCurrency(partialAmount)} (
                    {partialMethod.toUpperCase()})
                  </span>
                  {balanceDue > 0 && (
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-lg border p-3">
                <p className="text-sm italic">
                  "{selectedCustomer.name.split(" ")[0]} ji, aapka{" "}
                  {isProforma ? "quotation" : "bill"} {formatCurrency(total)}{" "}
                  ka. Dhanyavaad!"
                </p>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <strong>{creditLimitCtx?.customerName}</strong> has a credit limit
              of{" "}
              <strong>
                ₹{(creditLimitCtx?.limit ?? 0).toLocaleString("en-IN")}
              </strong>
              .<br />
              Current balance:{" "}
              <strong>
                ₹{(creditLimitCtx?.balance ?? 0).toLocaleString("en-IN")}
              </strong>
              <br />
              This invoice:{" "}
              <strong>
                ₹{(creditLimitCtx?.invoiceAmount ?? 0).toLocaleString("en-IN")}
              </strong>
              <br />
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setCreditLimitCtx(null)}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
    ...
    setItems((prev) =>
      prev.map((it, i) => {
        ...
        return {
          ...it,
          qty: String(qty),
          total: Math.round(effective * qty * 100) / 100,
        };
      }),
    );
  };
      ...
      <table className="w-full">
        <thead className="bg-muted/50">
          <tr>
            ...
            {showPriceEdit && (
              <th className="px-1 py-2 text-right font-medium w-20">Price</th>
            )}
                ...
                <div className="flex items-center justify-center gap-1">
                  <button
                    onClick={() =>
                      updateQty(
                        idx,
                        String(Math.max(1, parseInt(item.qty) - 1)),
                      )
                    }
                  ...
                  <button
                    onClick={() =>
                      updateQty(idx, String(parseInt(item.qty) + 1))
                    }
                    className="rounded p-0.5 hover:bg-muted"
                  >
        ...
        <tfoot>
          ...
          {discountAmt > 0 && (
            <tr className="bg-muted/30">
              <td
                colSpan={colCount - 1}
                className="px-3 py-1 text-right text-xs text-green-600"
              >
                Discount{" "}
                {discountPct > 0
                  ? `(${discountPct}%)`
          ...
          {partialAmount > 0 && (
            <tr className="bg-muted/30">
              ...
              <td
                colSpan={1}
                className="px-3 py-1.5 text-right text-xs font-medium text-destructive"
              >
                ₹{balanceDue}
              </td>
            </tr>
        ...
        <tfoot>
          ...
          {discountAmt > 0 && (
            <tr className="bg-muted/30">
              ...
              <td className="px-3 py-1 text-right text-green-600">
                -₹{discountAmt}
              </td>
            </tr>
          )}
