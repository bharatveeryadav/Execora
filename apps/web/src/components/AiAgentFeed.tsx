...
const COMMAND_SETS = [
	{
		category: '💰 Sales',
		items: ['Ramesh ka invoice banao 3 rice 50kg', 'Suresh ko 3 bag diya 1200 ka', 'Invoice print karo'],
	},
	{
		category: '💸 Payment',
		items: ['Ramesh ne 500 diya', 'Sita ka payment 2000 record karo', 'Aaj kitna collection hua?'],
	},
	{ category: '📦 Stock', items: ['Rice kitna bacha?', 'Atta ka stock low hai', 'Sugar 100kg add karo'] },
	{
		category: '👥 Customers',
		items: ['Ramesh ka balance batao', 'Suresh ka udhar kitna hai?', 'Naya customer Mohan add karo'],
	},
	{ category: '📊 Reports', items: ['Aaj ki sale kitni hui?', 'Is hafte ka report dikhao', 'GSTR-1 download karo'] },
	{
		category: '🧾 Misc',
		items: ['Kaunse customers ka pesa aana baaki hai?', 'Low stock alert dikhao', 'Business health kaisi hai?'],
	},
];
	...
	return (
		<div className="flex items-start gap-2.5 py-2">
			...
			<div className="min-w-0 flex-1">
				...
				{item.subtext && <p className="truncate text-[11px] text-muted-foreground">{item.subtext}</p>}
			</div>
...
const AiAgentFeed = () => {
	...
	const push = (item: Omit<FeedItem, 'id' | 'at'>) =>
		setFeed((prev) => [{ ...item, id: _counter++, at: Date.now() }, ...prev].slice(0, 20));

	// WS subscriptions
	useEffect(() => {
		const offs = [
			wsClient.on('invoice:confirmed', (p) => {
				const d = p as { invoiceNo?: string; customerName?: string; total?: number };
				push({
					type: 'invoice',
					icon: '🧾',
					...
					subtext: [
						d.invoiceNo,
						d.total ? `₹${parseFloat(String(d.total)).toLocaleString('en-IN')}` : undefined,
					]
						.filter(Boolean)
						.join('  ·  '),
				});
			...
			wsClient.on('payment:recorded', (p) => {
				...
				push({
					...
					subtext: d.amount ? `₹${parseFloat(String(d.amount)).toLocaleString('en-IN')} recorded` : undefined,
				});
				setActionCount((c) => c + 1);
			}),
			wsClient.on('customer:updated', (p) => {
				const d = p as { name?: string };
				push({ type: 'customer', icon: '👤', text: `Customer updated — ${d.name ?? ''}` });
				setActionCount((c) => c + 1);
			}),
			wsClient.on('product:updated', (p) => {
				const d = p as { name?: string };
				push({ type: 'product', icon: '📦', text: `Stock updated — ${d.name ?? 'Product'}` });
				setActionCount((c) => c + 1);
			}),
			wsClient.on('voice:thinking', (p) => {
				const d = p as { transcript?: string };
				if (d.transcript) {
					push({ type: 'voice', icon: '🎤', text: `"${d.transcript}"`, subtext: 'AI is processing…' });
					setVcCount((c) => c + 1);
				}
			}),
			wsClient.on('voice:response', (p) => {
				...
				if (d.text) {
					setFeed((prev) => {
						...
						const idx = prev.findIndex((r) => r.type === 'voice' && r.subtext === 'AI is processing…');
				...
				<div className="rounded-xl border bg-card px-4">
					<div className="flex items-center justify-between border-b py-2.5">
						<div className="flex items-center gap-2">
							<span
								className={`h-1.5 w-1.5 rounded-full ${isConnected ? 'bg-green-500 animate-pulse' : 'bg-muted'}`}
							/>
