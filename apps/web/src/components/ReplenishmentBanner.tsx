...
const URGENCY_COLOR: Record<Urgency, string> = {
	critical: 'bg-red-100 border-red-300 dark:bg-red-950 dark:border-red-700',
	high: 'bg-orange-100 border-orange-300 dark:bg-orange-950 dark:border-orange-700',
	normal: 'bg-yellow-50 border-yellow-300 dark:bg-yellow-950 dark:border-yellow-700',
};
...
export function ReplenishmentBanner() {
	...
	return (
		<div className={`rounded-lg border px-4 py-3 ${URGENCY_COLOR[headerUrgency]}`} role="alert">
			{/* ── Banner header ── */}
						...
						.map((s) => (
							...
							>
								<div className="min-w-0 flex-1">
									...
									{s.preferredSupplier && (
										<p className="truncate text-xs text-muted-foreground">{s.preferredSupplier}</p>
									)}
								</div>
								...
								<div className="flex shrink-0 flex-wrap items-center gap-2 text-xs">
									...
									<span className="font-semibold">Order: {s.suggestedOrderQty}</span>
