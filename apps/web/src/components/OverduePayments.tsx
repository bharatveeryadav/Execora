...
const OverduePayments = () => {
	...
	const getDaysOverdue = (customerId: string): number | null => {
		const unpaid = allInvoices.filter(
			(inv) => inv.customerId === customerId && inv.status !== 'paid' && inv.status !== 'cancelled'
		);
	...
	return (
		<Card className="border-none shadow-sm">
			<CardHeader className="flex flex-row items-center justify-between pb-3">
				...
				<Button variant="link" className="h-auto p-0 text-xs text-primary" onClick={() => navigate('/overdue')}>
					View All →
				</Button>
			</CardHeader>
			<CardContent className="p-0">
				<div className="divide-y">
					{isLoading ? (
						...
						<EmptyState icon="✅" title="All clear!" description="No overdue payments right now." compact />
					) : (
