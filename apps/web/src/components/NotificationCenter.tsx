...
import { Bell, X, AlertTriangle, Package, DollarSign, CheckCircle, ClipboardList } from 'lucide-react';
...
export default function NotificationCenter() {
	...
	const overdueCustomers = customers.filter((c) => parseFloat(String((c as any).balance ?? 0)) > 0);
	const totalOverdue = overdueCustomers.reduce((s, c) => s + parseFloat(String((c as any).balance ?? 0)), 0);
			...
			? [
					{
						...
						action: () => window.dispatchEvent(new CustomEvent('open-draft-panel')),
						actionLabel: 'Review',
