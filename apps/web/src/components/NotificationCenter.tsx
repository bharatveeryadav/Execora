...
import { Bell, X, AlertTriangle, Package, DollarSign, CheckCircle } from "lucide-react";
...
export default function NotificationCenter() {
      ...
      ? [
          {
            ...
            body: `${lowStock.length} items below minimum: ${lowStock.slice(0, 2).map((p) => p.name).join(", ")}${lowStock.length > 2 ? "…" : ""}`,
  ...
  const iconMap = {
    ...
    info: <AlertTriangle className="h-4 w-4 text-muted-foreground" />,
  };
        ...
        <div className="max-h-80 overflow-y-auto" style={{ scrollbarWidth: "thin" }}>
          {notifications.length === 0 ? (
            ...
            notifications.map((n) => (
              ...
              >
                <div className={cn("mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full", bgMap[n.type])}>
                  {iconMap[n.type]}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium leading-tight">{n.title}</p>
                  <p className="mt-0.5 text-xs text-muted-foreground leading-tight">{n.body}</p>
                  {n.action && (
                    <button
                      onClick={() => {
