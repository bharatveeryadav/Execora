...
function centred(s, cols) {
    const pad = Math.max(0, Math.floor((cols - s.length) / 2));
    return " ".repeat(pad) + s;
}
function buildReceiptHtml(data, config) {
    ...
    if (data.shopAddress)
        lines.push(centred(data.shopAddress.slice(0, cols), cols));
    if (data.shopPhone)
        lines.push(centred(`Ph: ${data.shopPhone}`, cols));
    lines.push(hr);
    ...
    for (const item of data.items) {
        ...
        if (item.discountPct > 0)
            detail += ` (-${item.discountPct}%)`;
        lines.push(detail.slice(0, cols));
    }
    lines.push(hr);
    const totalRow = (label, value) => padR("  " + label, itemW) + " " + padL(value, amtW);
    ...
    for (const p of data.payments) {
        if (p.amount > 0)
            lines.push(totalRow(p.mode, INR(p.amount)));
    }
    lines.push(hr);
