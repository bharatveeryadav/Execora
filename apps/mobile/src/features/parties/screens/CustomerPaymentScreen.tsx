...
export function PaymentScreen({ navigation, route }: Props) {
  ...
  const canSubmit =
    !!selectedCustomerId && isAmountValid && !recordMutation.isPending;
      ...
      <View style={{ flex: 1, width: "100%", alignItems: "center" }}>
        <View style={{ width: "100%", flex: 1 }}>
          ...
          <View
            ...
            className="bg-white border-b border-slate-100 flex-row items-center"
          >
            <TouchableOpacity
          ...
          >
            <ScrollView
              className="flex-1"
              contentContainerStyle={{ padding: contentPad, gap: 14 }}
              keyboardShouldPersistTaps="handled"
            >
              {/* Customer selector */}
              <View className="bg-white rounded-2xl p-4 shadow-sm">
                <Text className="text-xs font-semibold uppercase text-slate-400 mb-3">
                  Customer
                </Text>
                ...
                {selectedCustomerId && customer ? (
                  ...
                  <View className="flex-row items-center">
                    ...
                    <View className="flex-1">
                      ...
                      {balance !== 0 && (
                        <Text
                          className={`text-xs font-medium mt-0.5 ${balance > 0 ? "text-red-600" : "text-green-600"}`}
                        >
                          {balance > 0
                        ...
                        {searchResults.map((c: any, i: number) => (
                          <TouchableOpacity
                            ...
                            onPress={() => {
                              ...
                              setSearch("");
                              if (c.balance > 0) setAmount(String(c.balance));
                            }}
                          ...
                          >
                            ...
                            {c.balance > 0 && (
                              <Text className="text-xs text-red-500 font-semibold">
                                {formatCurrency(toFloat(c.balance))} due
                              </Text>
                            )}
                          </TouchableOpacity>
                        ))}
              ...
              <View className="bg-white rounded-2xl p-4 shadow-sm">
                ...
                <View className="flex-row items-center border border-slate-200 rounded-xl bg-slate-50 px-3">
                  <Text className="text-slate-600 text-lg font-semibold mr-2">
                    ₹
                  </Text>
                  <TextInput
                    value={amount}
              ...
              <View className="bg-white rounded-2xl p-4 shadow-sm">
                ...
                <View className="flex-row flex-wrap gap-2">
                  {PAYMENT_METHODS.map((m) => (
                    ...
                    >
                      <Text
                        className={`text-sm font-medium ${method === m.value ? "text-white" : "text-slate-600"}`}
                      >
                        {m.label}
                      </Text>
                    </TouchableOpacity>
              ...
              <View className="bg-white rounded-2xl p-4 shadow-sm">
                ...
                <TextInput
                  ...
                  placeholderTextColor="#94a3b8"
                  multiline
                  className="border border-slate-200 rounded-xl px-3 py-3 text-sm text-slate-800 min-h-[70px]"
                />
              </View>

              {/* Summary */}
              {selectedCustomerId && isAmountValid && (
                <View className="bg-primary/10 border border-primary/30 rounded-2xl p-4">
                  <Text className="text-xs font-semibold uppercase text-primary-400 mb-2">
                    Summary
                  </Text>
                  <View className="gap-1">
                    ...
                    <View className="flex-row justify-between">
                      ...
                      <Text className="text-sm font-semibold text-primary-800 capitalize">
                        {method.replace("_", " ")}
                      </Text>
                    </View>
                      ...
                      parsedAmount < balance && (
                        <View className="mt-2 bg-yellow-50 rounded-xl px-3 py-2">
                          <Text className="text-xs text-yellow-700 text-center">
                            Remaining: {formatCurrency(balance - parsedAmount)}
                          </Text>
                        </View>
                      )}
                  </View>
          ...
          <View
            ...
            className="bg-white border-t border-slate-100 py-4"
          >
            <TouchableOpacity
            ...
            >
              {recordMutation.isPending ? (
                ...
                <Text
                  className={`font-bold text-base ${canSubmit ? "text-white" : "text-slate-400"}`}
                >
                  💰 Record Payment
                </Text>
              )}
