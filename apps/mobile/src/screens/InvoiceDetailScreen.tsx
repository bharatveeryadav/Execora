...
import { STATUS_STYLES, PAYMENT_METHODS } from "../lib/constants";
...
import { showInfo, showSuccess } from "../lib/alerts";
...
export function InvoiceDetailScreen({ navigation, route }: Props) {
  ...
  // ── Queries & Mutations ───────────────────────────────────────────────────
  const { data: invoice, isLoading, refetch, isFetching } = useInvoiceById(id);
  ...
  const statusCfg =
    STATUS_STYLES[invoice.status as keyof typeof STATUS_STYLES] ??
      ...
      <View style={{ flex: 1, width: "100%", alignItems: "center" }}>
        <View style={{ width: "100%", maxWidth: contentWidth, flex: 1 }}>
          ...
          <View
            ...
            className="bg-white border-b border-slate-100 flex-row items-center justify-between"
          >
            <TouchableOpacity
              onPress={() => navigation.goBack()}
              className="mr-3 p-1"
            ...
            <View className="flex-row items-center gap-2">
              <View className={`px-2 py-1 rounded-full ${statusCfg.bg}`}>
                <Text className={`text-[11px] font-semibold ${statusCfg.text}`}>
                  {statusCfg.label}
                </Text>
              </View>
              {canEdit && (
            ...
            <View className="bg-white rounded-2xl p-5 shadow-sm">
              <View className="flex-row items-start justify-between">
                <View>
                  ...
                  <Text className="text-3xl font-black text-slate-800 mt-0.5">
              ...
              <View>
                ...
                <View className="bg-white rounded-2xl overflow-hidden shadow-sm">
                  ...
                  <View className="flex-row bg-slate-50 px-4 py-2">
                    <Text className="flex-1 text-[11px] font-semibold uppercase text-slate-400">
                      Item
                    </Text>
                    <Text className="w-10 text-[11px] font-semibold uppercase text-slate-400 text-right">
                      Qty
                    </Text>
                    <Text className="w-20 text-[11px] font-semibold uppercase text-slate-400 text-right">
                      Rate
                    </Text>
                    <Text className="w-20 text-[11px] font-semibold uppercase text-slate-400 text-right">
                      Amount
                  ...
                  {invoice.items.map((item: any, i: number) => (
                    ...
                    >
                      ...
                      <Text className="w-10 text-sm text-slate-600 text-right">
                        {item.quantity}
                      </Text>
                      <Text className="w-20 text-sm text-slate-600 text-right">
                        {formatCurrency(toFloat(item.unitPrice))}
                      </Text>
            ...
            <ScrollView>
              {editItems.map((item, idx) => {
                ...
                return (
                  <View
                    key={item.id}
                    className="flex-row items-center py-2 border-b border-slate-50 gap-2"
                  >
                    <Text
                    ...
                    <Text className="w-8 text-center font-semibold text-slate-800">
                      {item.qty}
                    </Text>
                    <TouchableOpacity
                      onPress={() => changeQty(idx, 1)}
                    ...
                    <Text className="w-20 text-right text-sm font-semibold text-slate-800">
        ...
        <View className="flex-1 bg-black/50 justify-end">
          <View className="bg-white rounded-t-3xl px-5 pt-5 pb-8">
            ...
            <Text className="text-sm text-slate-500 mb-4">
              Optionally record an initial payment below.
            </Text>
            ...
            <View className="flex-row flex-wrap gap-2 mb-4">
              {PAYMENT_METHODS.map((m) => (
                ...
                >
                  <Text
                    className={`text-sm font-medium ${convertMethod === m.value ? "text-white" : "text-slate-600"}`}
                  >
                    {m.label}
                  </Text>
                </TouchableOpacity>
            ...
            <View className="flex-row gap-3">
              ...
              <TouchableOpacity
                onPress={() => {
                  ...
                  showInfo("Proforma conversion submitted", "Convert");
