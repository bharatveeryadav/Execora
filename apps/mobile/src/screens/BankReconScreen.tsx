    ...
    return (
      <SafeAreaView className="flex-1 bg-background">
        <View className="flex-1 justify-center px-4">
          <ErrorCard message="Failed to load cashbook" onRetry={() => refetch()} />
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-background" edges={["bottom"]}>
      <View className="px-4 pt-4 pb-3 border-b border-slate-200 bg-card">
        <Text className="text-xl font-bold tracking-tight text-slate-800 mb-3">Bank Reconciliation</Text>
        <View className="flex-row gap-2">
          ...
          <Chip label="2 Months" selected={period === "2month"} onPress={() => setPeriod("2month")} />
        </View>
      </View>

      <View className="px-4 py-4 bg-slate-50">
        <Text className="text-sm text-slate-600">Net Cash ({from} to {to})</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          isFetching ? (
            ...
            <Text className="text-slate-500 text-center py-8">No entries in this period</Text>
          )
        }
      />
    </SafeAreaView>
