...
import { sttService } from '@execora/modules';
import { ttsService } from '@execora/modules';
...
const SYSTEM_MSGS: Record<string, { repeat: string; askYesNo: string; cancelled: string; confirmSuffix: string }> = {
	hi: {
		repeat: 'Dobara boliye, samajh nahi aaya.',
		askYesNo: 'Haan ya nahi boliye.',
		cancelled: 'Theek hai, cancel kiya.',
		confirmSuffix: 'Haan ya nahi?',
	},
	'hi-en': {
		repeat: 'Dobara boliye, samajh nahi aaya.',
		askYesNo: 'Haan ya nahi boliye.',
	...
	en: {
		repeat: 'Please repeat, I did not understand.',
		askYesNo: 'Please say yes or no.',
		cancelled: 'Okay, cancelled.',
		confirmSuffix: 'Yes or no?',
	},
	bn: {
		repeat: 'আবার বলুন, বুঝতে পারিনি।',
		askYesNo: 'হ্যাঁ বা না বলুন।',
		cancelled: 'ঠিক আছে, বাতিল।',
		confirmSuffix: 'হ্যাঁ বা না?',
	},
	ta: {
		repeat: 'மீண்டும் சொல்லுங்கள், புரியவில்லை.',
		askYesNo: 'ஆம் அல்லது இல்லை சொல்லுங்கள்.',
		cancelled: 'சரி, ரத்து செய்யப்பட்டது.',
		confirmSuffix: 'ஆம் அல்லது இல்லை?',
	},
	te: {
		repeat: 'మళ్ళీ చెప్పండి, అర్థం కాలేదు.',
		askYesNo: 'అవును లేదా కాదు చెప్పండి.',
		cancelled: 'సరే, రద్దు చేయబడింది.',
		confirmSuffix: 'అవును లేదా కాదు?',
	},
	mr: {
		repeat: 'पुन्हा सांगा, समजले नाही.',
		askYesNo: 'हो किंवा नाही सांगा.',
		cancelled: 'ठीक आहे, रद्द केले.',
		confirmSuffix: 'हो किंवा नाही?',
	},
	gu: {
		repeat: 'ફરી કહો, સમજ ન આવ્યું.',
		askYesNo: 'હા અથવા ના કહો.',
		cancelled: 'ઠીક છે, રદ કર્યું.',
		confirmSuffix: 'હા અથવા ના?',
	},
	kn: {
		repeat: 'ಮತ್ತೆ ಹೇಳಿ, ಅರ್ಥವಾಗಲಿಲ್ಲ.',
		askYesNo: 'ಹೌದು ಅಥವಾ ಇಲ್ಲ ಹೇಳಿ.',
		cancelled: 'ಸರಿ, ರದ್ದು ಮಾಡಲಾಗಿದೆ.',
		confirmSuffix: 'ಹೌದು ಅಥವಾ ಇಲ್ಲ?',
	},
	pa: {
		repeat: 'ਦੁਬਾਰਾ ਕਹੋ, ਸਮਝ ਨਹੀਂ ਆਇਆ।',
		askYesNo: 'ਹਾਂ ਜਾਂ ਨਹੀਂ ਕਹੋ।',
		cancelled: 'ਠੀਕ ਹੈ, ਰੱਦ ਕੀਤਾ।',
		confirmSuffix: 'ਹਾਂ ਜਾਂ ਨਹੀਂ?',
	},
	ml: {
		repeat: 'വീണ്ടും പറയൂ, മനസ്സിലായില്ല.',
		askYesNo: 'അതെ അല്ലെങ്കിൽ അല്ല പറയൂ.',
		cancelled: 'ശരി, റദ്ദാക്കി.',
		confirmSuffix: 'അതെ അല്ലെങ്കിൽ അല്ല?',
	},
	ur: {
		repeat: 'دوبارہ بولیں، سمجھ نہیں آیا۔',
		askYesNo: 'ہاں یا نہیں کہیں۔',
		cancelled: 'ٹھیک ہے، منسوخ کیا۔',
		confirmSuffix: 'ہاں یا نہیں؟',
	},
	ar: {
		repeat: 'كرر من فضلك، لم أفهم.',
		askYesNo: 'قل نعم أو لا.',
		cancelled: 'حسناً، تم الإلغاء.',
		confirmSuffix: 'نعم أم لا؟',
	},
	es: {
		repeat: 'Repite por favor, no entendí.',
		askYesNo: 'Di sí o no.',
		cancelled: 'De acuerdo, cancelado.',
		confirmSuffix: '¿Sí o no?',
	},
	fr: {
		repeat: "Répétez s'il vous plaît, je n'ai pas compris.",
		askYesNo: 'Dites oui ou non.',
		cancelled: "D'accord, annulé.",
		confirmSuffix: 'Oui ou non?',
	},
};
...
class EnhancedWebSocketHandler {
	...
	async handleConnection(connection: WebSocket, request: FastifyRequest) {
		...
		connection.on('message', async (data: RawData, isBinary: boolean) => {
			try {
				...
				logger.info({ sessionId, messageType: message.type }, 'JSON message parsed');
	...
	private async handleAudioData(sessionId: string, audioChunk: Buffer) {
		...
		if (!session || !session.isActive) {
			logger.warn(
				{ sessionId, sessionExists: !!session, isActive: session?.isActive },
				'Audio received but session not active'
			);
			return;
		}
	...
	private async handleMessage(sessionId: string, message: any) {
		...
		switch (message.type) {
			...
			default:
				logger.warn(
					{ sessionId, type: message.type, allKeys: Object.keys(message) },
	...
	private async handleVoiceStart(session: VoiceSession, messageData?: any) {
		...
		try {
			...
			const clientAudioFormat: 'pcm' | 'webm' | 'ogg' = messageData?.audioFormat === 'pcm' ? 'pcm' : 'webm';
			...
			session.sttConnection = await sttService.createLiveTranscription(
				(text: string, isFinal: boolean) => {
					// Handle transcript
					logger.debug(
						{ sessionId: session.sessionId, textLength: text.length, isFinal },
						'STT callback: received transcript'
					);
					if (isFinal) {
						session.transcript = text;
						logger.info(
							{ sessionId: session.sessionId, text, isFinal: true },
							'Final transcript received from STT'
						);
						...
						// Interim results
						logger.debug(
							{ sessionId: session.sessionId, text, isFinal: false },
	...
	private async processFinalTranscript(session: VoiceSession, text: string, ttsProvider: string = 'browser') {
		...
		try {
			...
			if (intent.confidence < CONFIDENCE_THRESHOLD_CONFIRM) {
				...
				logger.info(
					{ sessionId: session.sessionId, confidence: intent.confidence, intent: intent.intent },
					'Confidence too low — asking to repeat'
				);
				this.sendMessage(session.ws, {
					type: WSMessageType.VOICE_RESPONSE,
			...
			if (this.intentNeedsConfirmation(intent)) {
				...
				logger.info(
					{ sessionId: session.sessionId, confirmText, intent: intent.intent, confidence: intent.confidence },
			...
			if (executionResult.success) {
				...
				if (d.resolvedItems?.length && d.awaitingConfirm) {
					...
					logger.info(
						{ sessionId: session.sessionId, intent: intent.intent, itemCount: d.resolvedItems.length },
						'invoice:draft pushed immediately'
					);
				}
			...
			if (intent.intent === 'LIST_CUSTOMER_BALANCES' && responseTemplateService.canUseTemplate(intent.intent)) {
				response =
					responseTemplateService.generateFastResponse(intent.intent, executionResult) ||
			...
			if (ttsService.isAvailable() && ttsProvider !== 'browser') {
				...
				logger.info(
					{ sessionId: session.sessionId },
					'Browser speech selected - skipping server TTS generation'
				);
			}
	...
	private async handlePendingConfirmation(session: VoiceSession, text: string, ttsProvider: string) {
		...
		if (!isYes && !isNo) {
			...
			try {
				...
				if (
					maybeIntent.intent !== IntentType.UNKNOWN &&
					maybeIntent.confidence >= CONFIDENCE_THRESHOLD_CONFIRM
				) {
					logger.info(
						{
							sessionId: session.sessionId,
							pendingIntent: pendingIntent.intent,
							freshIntent: maybeIntent.intent,
						},
				...
				logger.warn(
					...
					'Fresh command detection failed during pending confirmation'
				);
			}
		...
		if (isNo) {
			...
			logger.info(
				{ sessionId: session.sessionId, intent: pendingIntent.intent },
				'User cancelled pending intent'
			);
			this.sendMessage(session.ws, {
				type: WSMessageType.VOICE_RESPONSE,
		...
		logger.info(
			{ sessionId: session.sessionId, intent: pendingIntent.intent },
			'User confirmed — executing pending intent'
		);
		try {
			const execStart = Date.now();
			...
			const responseTime = Date.now() - responseStart;
			logger.info(
				{ sessionId: session.sessionId, response, responseTimeMs: responseTime },
				'[CONFIRMED INTENT] Response generated'
			);
	...
	private async generateAndSendTTS(session: VoiceSession, text: string, ttsProvider?: string) {
		try {
			logger.info(
				{ sessionId: session.sessionId, textLength: text.length, ttsProvider },
				'Starting TTS generation'
			);

			const audioStream = await ttsService.generateSpeechStream(text, ttsProvider);
			logger.info(
				{ sessionId: session.sessionId, isBuffer: Buffer.isBuffer(audioStream) },
				'TTS stream received'
			);
			...
			if (Buffer.isBuffer(audioStream)) {
				...
				logger.info(
					...
					'TTS stream converted to buffer'
				);
			}
			...
			logger.info(
				{ sessionId: session.sessionId, audioSize: audioBuffer.length, provider: ttsProvider },
	...
	private async saveRecording(session: VoiceSession) {
		...
		try {
			...
			await voiceSessionService.saveRecording(session.conversationSessionId, audioBuffer, {
				fileName,
				mimeType: session.audioFormat === 'pcm' ? 'audio/pcm' : 'audio/webm',
			});

			logger.info({ sessionId: session.sessionId, size: audioBuffer.length }, 'Recording saved');
