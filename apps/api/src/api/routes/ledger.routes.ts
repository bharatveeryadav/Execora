...
export async function ledgerRoutes(fastify: FastifyInstance) {
  fastify.post('/api/v1/ledger/payment', {
    ...
    Body: { customerId: string; amount: number; paymentMode: 'cash' | 'upi' | 'card' | 'other'; notes?: string };
  }>) => {
    const entry = await ledgerService.recordPayment(request.body.customerId, request.body.amount, request.body.paymentMode, request.body.notes);
    return { entry };
  });
  ...
  fastify.post('/api/v1/ledger/credit', {
    ...
    const entry = await ledgerService.addCredit(request.body.customerId, request.body.amount, request.body.description);
    return { entry };
  });
  ...
  fastify.get('/api/v1/ledger/:customerId', async (request: FastifyRequest<{
    ...
    const entries = await ledgerService.getCustomerLedger(request.params.customerId, parseLimit(request.query.limit, 50));
    return { entries };
  });
}
