...
export async function summaryRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/api/v1/summary/daily', async (request: FastifyRequest<{
  ...
  fastify.get('/api/v1/summary/range', async (request: FastifyRequest<{
    Querystring: { from?: string; to?: string };
  }>, reply) => {
    const { from, to } = request.query;
    if (!from || !to) {
      return reply.code(400).send({ error: 'Both from and to query params are required (YYYY-MM-DD)' });
    }
    ...
    const summary = await invoiceService.getSummaryRange(fromDate, toDate);
    return { summary };
  });
}
