...
export async function usersRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
        ...
        body: {
          ...
          properties: {
            ...
            permissions: {
              ...
              items: { type: "string" },
              description:
                "Override default role permissions. Leave empty to use role defaults.",
            },
          },
    ...
    async (
      ...
      if (role === "admin" && request.user!.role !== "owner") {
        return reply
          .code(403)
          .send({ error: "Only owner can create admin-level users" });
      }
      ...
      logger.info(
        { createdBy: request.user!.userId, newUserId: user.id, role },
        "User created",
      );
    ...
    async (request, reply) => {
      ...
      if (request.body.role !== undefined) {
        if (request.body.role === "owner") {
          return reply
            .code(403)
            .send({ error: "Cannot assign owner role via API" });
        }
      ...
      logger.info(
        { updatedBy: callerId, targetId, changes: Object.keys(data) },
        "User updated",
      );
  ...
  fastify.delete<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (target.role === "owner") {
        return reply
          .code(403)
          .send({ error: "Cannot deactivate owner account" });
      }
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      logger.info(
        { resetBy: callerId, targetId: request.params.id },
        "User password reset by owner",
      );
      return reply.send({ success: true });
    },
  );
