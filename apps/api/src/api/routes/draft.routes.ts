...
export async function draftRoutes(fastify: FastifyInstance) {
	...
	fastify.post('/api/v1/drafts/:id/confirm', async (request: FastifyRequest<{ Params: { id: string } }>, reply) => {
		...
		try {
			...
			if (draft.type === 'purchase_entry') broadcaster.send(tid, 'purchase:created', r);
			else if (draft.type === 'product') broadcaster.send(tid, 'product:created', r);
			else if (draft.type === 'stock_adjustment') broadcaster.send(tid, 'product:updated', r);
			broadcaster.send(tid, 'draft:confirmed', { draftId: draft.id, type: draft.type, title: draft.title });
			return { draft, result };
		} catch (err: any) {
			fastify.log.error({ draftId: request.params.id, err }, 'Draft confirm failed');
			const is404 = /not found/i.test(err.message);
			const code = is404 ? 404 : /already/i.test(err.message) ? 409 : 422;
			return reply.code(code).send({ error: err.message ?? 'Confirm failed' });
		}
	});
