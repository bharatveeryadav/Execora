...
export async function authRoutes(fastify: FastifyInstance) {
    ...
    async (
      ...
      if (!isPasswordSet(user.passwordHash)) {
        return reply.code(401).send({
          ...
          message:
            "Password has not been set for this account. Contact your administrator.",
        });
      }
  ...
  fastify.put(
    ...
    async (
      ...
      if (request.body.preferences !== undefined)
        userData.preferences = request.body.preferences;
      ...
      if (tenantBody) {
        ...
        if (tenantBody.legalName !== undefined)
        ...
        if (tenantBody.timezone !== undefined)
          tenantData.timezone = tenantBody.timezone;
        if (tenantBody.language !== undefined)
          tenantData.language = tenantBody.language;
        if (tenantBody.dateFormat !== undefined)
          tenantData.dateFormat = tenantBody.dateFormat;
        if (tenantBody.logoUrl !== undefined)
          tenantData.logoUrl = tenantBody.logoUrl || null;
      ...
      logger.info(
        {
          ...
          tenantFields: Object.keys(tenantData),
        },
        "Profile updated",
      );
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (role !== "owner" && role !== "admin") {
        return reply
          .code(403)
          .send({ error: "Only owner/admin can update company logo" });
      }
  ...
  fastify.post(
    ...
    async (request: FastifyRequest<{ Body: { password: string } }>, reply) => {
      ...
      return reply.send({
        hash,
        usage: "Set ADMIN_PASSWORD_HASH=<hash> in .env",
      });
    },
  );
}
