...
// ── Users ──────────────────────────────────────────────────────────────────
export const adminGetUsers = (params?: { page?: number; limit?: number; q?: string; tenantId?: string; role?: string }) =>
...
export const adminUpdateUserStatus = (id: string, isActive: boolean) =>
  adminFetch<{ success: boolean; isActive: boolean }>(`/admin/users/${id}/status`, {
    method: "PUT",
    body: JSON.stringify({ isActive }),
  });
export const adminGetUserSessions = (id: string) =>
  adminFetch<{ data: AdminUserSession[] }>(`/admin/users/${id}/sessions`);
export const adminRevokeUserSessions = (id: string) =>
  adminFetch<{ success: boolean; revokedCount: number }>(`/admin/users/${id}/sessions`, { method: "DELETE" });

// ── Tenants ────────────────────────────────────────────────────────────────
export const adminGetTenants = (params?: { page?: number; limit?: number; q?: string; plan?: string; status?: string }) =>
  adminFetch<AdminListResponse<AdminTenant>>(`/admin/tenants?${buildQs(params)}`);
export const adminGetTenant = (id: string) =>
  adminFetch<{ tenant: AdminTenantDetail }>(`/admin/tenants/${id}`);
...
export const adminUpdateTenant = (id: string, body: {
  name?: string; plan?: string; status?: string; gstin?: string; legalName?: string;
}) => adminFetch<{ tenant: AdminTenant }>(`/admin/tenants/${id}`, { method: "PUT", body: JSON.stringify(body) });
export const adminSuspendTenant = (id: string) =>
  adminFetch<{ tenant: AdminTenant }>(`/admin/tenants/${id}`, {
    method: "PUT", body: JSON.stringify({ status: "suspended" }),
  });
export const adminActivateTenant = (id: string) =>
  adminFetch<{ tenant: AdminTenant }>(`/admin/tenants/${id}`, {
    method: "PUT", body: JSON.stringify({ status: "active" }),
  });
export const adminDeleteTenant = (id: string, confirm: string) =>
  adminFetch<{ success: boolean }>(`/admin/tenants/${id}`, {
...
// ── Analytics API ──────────────────────────────────────────────────────────
export const adminGetAnalyticsRevenue = (days = 30) =>
  adminFetch<{ data: AdminRevenuePoint[]; period: number }>(`/admin/analytics/revenue?days=${days}`);
export const adminGetTopTenants = (limit = 10) =>
  adminFetch<{ data: AdminTopTenant[] }>(`/admin/analytics/top-tenants?limit=${limit}`);
export const adminGetAnalyticsTenants = () =>
  adminFetch<AdminTenantAnalytics>("/admin/analytics/tenants");

// ── Activity log API ───────────────────────────────────────────────────────
export const adminGetActivity = (params?: { page?: number; limit?: number; tenantId?: string; action?: string; entityType?: string }) =>
  adminFetch<AdminListResponse<AdminActivityEntry>>(`/admin/activity?${buildQs(params)}`);

// ── Super Admin: Announcements ─────────────────────────────────────────────
export const adminGetAnnouncements = () =>
  adminFetch<{ data: AdminAnnouncement[] }>("/admin/announcements");
export const adminCreateAnnouncement = (body: { title: string; message: string; level: AdminAnnouncement["level"]; expiresAt?: string }) =>
  adminFetch<{ announcement: AdminAnnouncement }>("/admin/announcements", {
    method: "POST",
    body: JSON.stringify(body),
  });
export const adminDeleteAnnouncement = (id: string) =>
  adminFetch<{ success: boolean }>(`/admin/announcements/${id}`, { method: "DELETE" });

// ── Super Admin: Maintenance mode ──────────────────────────────────────────
export const adminGetMaintenanceStatus = () =>
  adminFetch<AdminMaintenanceStatus>("/admin/maintenance");
export const adminSetMaintenance = (enabled: boolean, reason?: string) =>
  adminFetch<AdminMaintenanceStatus>("/admin/maintenance", {
    method: "PUT",
...
export interface AdminUserSession {
  id: string; ipAddress?: string; userAgent?: string;
