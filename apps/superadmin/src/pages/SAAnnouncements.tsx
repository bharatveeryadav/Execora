...
import {
  saGetAnnouncements, saCreateAnnouncement, saDeleteAnnouncement,
  SAAnnouncement,
} from "@/lib/sa-api";
...
const LEVELS: { value: SAAnnouncement["level"]; label: string; color: string }[] = [
...
export default function SAAnnouncements() {
  ...
  const [form, setForm] = useState({
    title: "", message: "", level: "info" as SAAnnouncement["level"], expiresAt: "",
  });
  const [err, setErr] = useState("");

  const { data, refetch, isLoading } = useQuery({
    queryKey: ["sa-announcements"],
    queryFn: saGetAnnouncements,
  });

  const createMut = useMutation({
    mutationFn: () => saCreateAnnouncement({
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) => saDeleteAnnouncement(id),
    onSuccess: () => refetch(),
  });
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        {data?.data?.map((ann: SAAnnouncement) => {
          const lc = LEVELS.find((l) => l.value === ann.level) ?? LEVELS[0];
          return (
            <div key={ann.id} className={`rounded-xl border p-4 ${lc.color}`}>
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1 flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${lc.color}`}>
                      {lc.label}
                    </span>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-gray-800 border border-gray-700 text-white rounded-md px-3 py-2 text-sm focus:outline-none focus:border-amber-500 resize-none"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    value={form.level}
                    onChange={(e) => setForm({ ...form, level: e.target.value as SAAnnouncement["level"] })}
                    className="w-full bg-gray-800 border border-gray-700 text-white rounded-md px-3 py-2 text-sm focus:outline-none focus:border-amber-500"
                  >
