 ...
 *
 * Run with:  npm run worker        (dev — tsx watch)
 *            npm run start:worker  (production — compiled JS)
 *
 * All worker logic lives in src/infrastructure/workers.ts — this file is
 ...
 * by the main API server (src/index.ts → startWorkers()), as both consume
