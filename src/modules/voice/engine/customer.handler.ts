...
export async function executeTotalPendingAmount(): Promise<ExecutionResult> {
...
export async function executeListCustomerBalances(): Promise<ExecutionResult> {
...
export async function executeCreateCustomer(
  ...
  if (amount) await customerService.updateBalance(customer.id, Number(amount));
...
export async function executeUpdateCustomer(
  ...
  try {
    ...
    const { customer, multiple } = await resolveCustomer(entities, conversationId);
    if (!customer) return { success: false, message: 'Customer nahi mila.', error: 'CUSTOMER_NOT_FOUND' };
    if (multiple) return { success: false, message: `Kai customers hain "${entities.customer}" naam se. Landmark ke saath batao.`, error: 'AMBIGUOUS_CUSTOMER' };

    const updated = await customerService.updateCustomer(customer.id, updates);
    if (!updated) return { success: false, message: 'Update nahi ho saka. Dobara try karo.', error: 'UPDATE_FAILED' };
    ...
    const lines = Object.entries(updates)
      .map(([k, v]) => `${FIELD_LABELS[k] ?? k}: ${k === 'phone' ? openaiService.phoneToWords(String(v)) : v}`)
      .join('\n');
...
export async function executeGetCustomerInfo(
  entities: Record<string, any>,
  conversationId?: string,
): Promise<ExecutionResult> {
  try {
...
export async function executeDeleteCustomerData(
  entities: Record<string, any>,
  conversationId?: string,
): Promise<ExecutionResult> {
  try {
    const isAdmin = entities?.operatorRole === 'admin' || !!entities?.adminEmail;
    if (!isAdmin) return { success: false, message: 'Yeh admin ke liye hai', error: 'UNAUTHORIZED' };

    const adminEmail = entities?.adminEmail || process.env.ADMIN_EMAIL;
    const resolution = await resolveCustomer(entities, conversationId);
    if (!resolution.customer) return { success: false, message: 'Customer not found', error: 'NOT_FOUND' };
    ...
    if (!result.success) return { success: false, message: 'Delete failed', error: result.error };
