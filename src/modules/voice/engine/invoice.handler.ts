...
export async function executeCreateInvoice(
  ...
  if (entities.autoSend) {
    ...
    return sendConfirmedInvoiceEmail(invoice, customer.id, customer.email || null, customer.name, preview.resolvedItems, conversationId);
  }
...
export async function executeConfirmInvoice(
  ...
  const result    = await sendConfirmedInvoiceEmail(invoice, draft.customerId, draft.customerEmail, draft.customerName, draft.resolvedItems, conversationId);
...
export async function executeProvideEmail(
  ...
  await emailService.sendInvoiceEmail(rawEmail, pending.customerName, pending.invoiceId, pending.items, pending.total, shopName, undefined, freshPdfUrl, pending.invoiceNo || pending.invoiceId);
